...
parser.add_argument('--pem-file', type=str, default="conf/pdb-key.pem",
                    help="the pem key file to connect to the cluster nodes, required only when running in distributed "
                         "mode (default: conf/pdb-key.pem)")
...
if cluster_type == "distributed":
    print(BColor.BOLD + "        pem file: " + BColor.END_C + pem_file)
...
def start_pseudo_cluster():
    try:
        ...
        if not os.path.isfile('bin/pdb-manager'):
            ...
            for each_line in f:
                if "#" not in each_line.strip() and len(each_line.strip()) > 0:
                    ...
                    if (os.path.isfile('bin/pdb-worker') == False):
                        print(
                                    BColor.FAIL + "PlinyCompute worker executable 'bin/pdb-worker' does not exist." + BColor.END_C)
                        sys.exit()
...
def download_tpch_data():
    ...
    if not os.path.isdir('tables_scale_0.2'):
        os.system(
            'wget https://www.dropbox.com/s/cl67ercyd0cm32p/tables_scale_0.2.tar.bz2?dl=0 -O tables_scale_0.2.tar.bz2')
    ...
    try:
        ...
        if cluster_type == "standalone":
            ...
            exit_code = subprocess.call(
                ['bash', './scripts/startCluster.sh', cluster_type, manager_ip, pem_file, thread_num,
                 shared_memory_size])
            if (exit_code != 0):
...
tests_int = {
    ...
    "TestAggregationAfterThreeWayJoin": ("SELECTION AND JOIN MIXED TEST ON G-2 PIPELINE",
                                         ['bin/TestAggregationAfterThreeWayJoin', 'Y', 'Y', '1024', 'localhost', 'Y']),
    "TestAggregationOnDoubleVector": ("AGGREGATION ON A DOUBLE VECTOR TEST G-2 PIPELINE",
                                      ['bin/TestAggregationOnDoubleVector', 'Y', 'Y', '1024', 'localhost', 'Y']),
    "TestAggregationWithoutCombiner": ("AGGREGATION WITHOUT COMBINER TEST G-2 PIPELINE",
                                       ['bin/TestAggregationWithoutCombiner', 'Y', 'Y', '1024', 'localhost', 'Y']),
    "TestAllSelection": (
    "ALL SELECTION TEST G-2 PIPELINE", ['bin/TestAllSelection', 'Y', 'Y', '256', 'localhost', 'Y']),
    "TestAllSelectionBuiltIn": (
    "ALL SELECTION BUILTIN TEST G-2 PIPELINE", ['bin/TestAllSelectionBuiltIn', 'Y', 'Y', '256', 'localhost', 'Y']),
    "TestAllSelectionShared": (
    "ALL SELECTION SHARED TEST G-2 PIPELINE", ['bin/TestAllSelectionShared', 'Y', 'Y', '256', 'localhost', 'Y']),
    "TestConsecutiveJoinWithTwoSinks": ("CONSECUTIVE JOIN WITH TWO SINKS TEST G-2 PIPELINE",
                                        ['bin/TestConsecutiveJoinWithTwoSinks', 'Y', 'Y', '1024', 'localhost', 'Y']),
    "TestConsecutiveOptimizedJoin": ("CONSECUTIVE OPTIMIZED JOIN TEST G-2 PIPELINE",
                                     ['bin/TestConsecutiveOptimizedJoin', 'Y', 'Y', '1024', 'localhost', 'Y']),
    "TestGroupByAggregation": (
    "GROUP BY AGGREGATION TEST G-2 PIPELINE", ['bin/TestGroupByAggregation', 'Y', 'Y', '1024', 'localhost', 'Y']),
    "TestIterativeAggregation": (
    ...
    "TestMultiSelectionWithJoin": ("MULTI SELECTION WITH JOIN TEST G-2 PIPELINE",
                                   ['bin/TestMultiSelectionWithJoin', 'Y', 'Y', '1024', 'localhost', 'Y']),
    "TestOptimizedGroupByAggregation": ("OPTIMIZED GROUP BY AGGREGATION TEST G-2 PIPELINE",
                                        ['bin/TestOptimizedGroupByAggregation', 'Y', 'Y', '1024', 'localhost', 'Y']),
    "TestOptimizedMethodJoin": (
    ...
    "TestSimpleSelectionWithRatio": ("SIMPLE SELECTION WITH RATIO TEST G-2 PIPELINE",
                                     ['bin/TestSimpleSelectionWithRatio', 'Y', 'Y', '1024', 'localhost', 'Y', '0.01']),
    "TestTwoSelectionOneAggregation": ("AGGREGATION AND SELECTION MIXED TEST ON G-2 PIPELINE",
                                       ['bin/TestTwoSelectionOneAggregation', 'Y', 'Y', '1024', 'localhost', 'Y']),
...
# Linear algebra tests
tests_la = {
    "TestLA_unit01_Transpose": (
    "TEST LA01_TRANSPOSE", ['bin/TestLA_unit01_Transpose', 'Y', 'Y', '64', 'localhost', 'Y']),
    "TestLA_unit02_Add": ("TEST LA02_ADD", ['bin/TestLA_unit02_Add', 'Y', 'Y', '64', 'localhost', 'Y']),
    "TestLA_unit03_Substract": (
    "TEST LA03_SUBSTRACT", ['bin/TestLA_unit03_Substract', 'Y', 'Y', '64', 'localhost', 'Y']),
    "TestLA_unit04_Multiply": ("TEST LA04_MULTIPLY", ['bin/TestLA_unit04_Multiply', 'Y', 'Y', '64', 'localhost', 'Y']),
    "TestLA_unit05_MaxElement": (
    "TEST LA05_MAXELEMENT", ['bin/TestLA_unit05_MaxElement', 'Y', 'Y', '64', 'localhost', 'Y']),
    "TestLA_unit06_MinElement": (
    "TEST LA06_MINELEMENT", ['bin/TestLA_unit06_MinElement', 'Y', 'Y', '64', 'localhost', 'Y']),
    "TestLA_unit07_ScaleMultiply": (
    "TEST LA07_SCALEMULTIPLY", ['bin/TestLA_unit07_ScaleMultiply', 'Y', 'Y', '64', 'localhost', 'Y']),
    ...
    "TestLA_unit12_DuplicateRow": (
    "TEST LA12_DUPLICATEROW", ['bin/TestLA_unit12_DuplicateRow', 'Y', 'Y', '64', 'localhost', 'Y']),
    "TestLA_unit13_DuplicateCol": (
    "TEST LA13_DUPLICATECOL", ['bin/TestLA_unit13_DuplicateCol', 'Y', 'Y', '64', 'localhost', 'Y']),
    ...
    "TestLA_unit17_TransposeMultiply": (
    "TEST LA17_TRANSPOSEMULTIPLY", ['bin/TestLA_unit17_TransposeMultiply', 'Y', 'Y', '64', 'localhost', 'Y']),
    "TestLA_unit18_TransposeMultiply_Gram": (
    "TEST LA18_TRANSPOSEMULTIPLY_GRAM", ['bin/TestLA_unit18_TransposeMultiply_Gram', 'Y', 'Y', '64', 'localhost', 'Y'])
}
