...
which requires an execution plan is constructed consisting entirely of calls to a toolkit of
...
To see how this works in practice, consider a variant of the \texttt{getSelection ()}:
...
\noindent PC may compile the lambda term resulting from \texttt{getSelection ()} into the following TCAP code:

\begin{codesmall}
WDNm_1(dep,emp,sup,nm1) <= 
  APPLY(In(dep), In(dep,emp,sup), 'Join_2212', 'att_acc_1');

WDNm_2(dep,emp,sup,nm1,nm2) <= 
   APPLY(WDNm_1(emp), WDNm_1(dep,emp,sup,nm1), 'Join_2212', 'method_call_2');
