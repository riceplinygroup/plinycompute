...
the PC API and TCAP compiler, and execution engine. 
%and we concretize these ideas by
%describing the implementation of a $k$-means clustering algorithm.
...
In addition, the compiled code referred to by \texttt{Join\_2212.att_acc\_1} will be executed via a vectorized application to the input
...
information can be vital during optimization.  For example, 
multiple calls to the same method can be detected using information
stored in the key-value maps, and the redundant calls can be eliminated.
