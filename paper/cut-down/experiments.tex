...
For the ten-dimensional computations, there was some variability in the results.
...
\vspace{5pt}
\noindent
\textbf{Data Representation.} To do this, we implement two different complex object computations on top of PC and on top of Spark.
...
\begin{codesmall}
Handle<Vector<int>> listOfPartIDs = makeObject<Vector<int>
    (199, 22, 34, 567, 1200, 37, 46, 459);
Handle<Computation> myReader = makeObject<ObjectReader
    <Customer>>("TPCH_db", "tpch_bench_set1");
Handle<Computation> myTopK = 
    makeObject<TopJaccard>(k, *listOfPartIDs);
myTopK->setInput(myReader);
Handle<Computation> myWriter = makeObject<Writer<TopKQueue
    <double,Handle<AllParts>>>>("TPCH_db", "result");
...
\vspace{5pt}
\noindent
\texttt{Latent Dirichlet Allocation.}
...
\vspace{5pt}
\noindent
\texttt{Gaussian Mixture Model.} A Gaussian mixture model (GMM) is a generative, statistical model where a dataset are modeled
...
\vspace{5pt}
\noindent
\texttt{$k$-Means.} $k$-means is a now-classic
...
\vspace{5pt}
\noindent
\textbf{Experiments: Final Thoughts.} The central hypothesis in this paper was that ``declarative in the large, high-performance in the small'' can result
in an excellent platform for tool and library development.  We believe that these experiments have shown that.  The
...
\hline
\end{tabular}
\caption{PlinyCompute vs. Spark: lines of source code (LOC) comparison.}
\label{fig:LOC}
\end{center}
\end{table*}
