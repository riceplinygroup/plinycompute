...
noReuse can speed up the processing time by $2\times$ to $5\times$ for
...
The difference is that it implements a \texttt{TopJaccard} class to customize the
\texttt{TopComp} declarative interface. Compared with \texttt{CustomerMultiSelection}, it
...
\texttt{MultiSelectionComp} interface. In addition, compared with the
\texttt{CustomerSupplierPartGroupBy} computation, the
\texttt{TopJaccard} bounds the intermediate data size to top K
elements, which also acause it generate less data. 
...
\caption{Single thread matrix multiplication benchmark, which shows
  ...
  $1000\times1000$ matrix multiplication, time are recorded in
  milliseconds, and for $10000\times10000$ matrix multiplication, time are
