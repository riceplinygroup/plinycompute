...
\item To test the utility of the PC object model, we first denormalized the TPC-H database \cite{} into an object-oriented 
...
development, most
(though not all) of our comparisons were with Spark (version XX.XX). The Spark cluster that we used... \textbf{Chris note:} can we say something
here about the Spark configuration?
...
we do not count the time required to load data from an external source into the system
(for example, 
...
read from HDFS (note that in each case, the data set was small enough to be entirely buffered in RAM by HDFS, so
reading from HDFS incurs no I/O time to access physical storage).
...
PC's storage system.  Again, since all data sets are small enough to be stored in RAM, there is no I/O time to
retrieve data.
...
a bit surmising because the top-$k$ computation seems, on the face of it, much easier 
...
that do integrate out these values are more common, but they such collapsed implementations
cannot be made ergodic in a distributed setting
(where ergodicity implies theoretical correctness in some sense).
Our implementation is
\emph{word based} because the fundamental data objects it operates over are \texttt{(docID, wordID, count)}  on top of
...
Our PC LDA computation makes use of the GSL library to perform all necessary random sampling (non-collapsed LDA requires 
...
by linear algebra operations (such as repeated vector-matrix multiples).
...
to the model happens; the result is broadcast in a new \texttt{AggregationComp} object, and the process begins again.
...
We installed OpenBLAS in the cluster, and \texttt{breeze} (used by the Spark \texttt{mllib} implementation)
invokes OpenBLAS' native code for numerical processing through JNI.
