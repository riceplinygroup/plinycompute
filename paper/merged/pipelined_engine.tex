...
To guarantee this,
PC obtains from Pangea a page for writing output objects for each thread, and places the current object allocation block
onto this page. 
...
on the output page.  The vector that results from extracting \texttt{Dep.deptName} from each input object is allocated outside of the output page, and stores C-style pointers to each \texttt{Dep.deptName}
...
All data produced via calls to \texttt{makeObject} are written to allocation blocks housed on in-memory pages served to PC by Pangaea.  PC's pipelined
...
input data from Pangea, and then repeatedly creates vectors of \texttt{Handle}s to the objects on
