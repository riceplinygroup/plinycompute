...
Note that there can be at most \emph{two} zombie output pages,
because pipelines are restricted 
to have just one sink.  
When a zombie output page is created, the vector list whose processing it
...
processed by the pipeline's sink, \emph{it cannot be used produce additional intermediate data whose references will be held by the vector list} 
...
store valid intermediate data and hence all zombie output pages can be flushed---and hnce the number of zombie output pages is capped at two.
