...
categorized into two classes for convenience of discussions: systems and techniques with a managed
runtime, and without a managed runtime.


\subsection {Optimizations with Managed Runtime}
...
code. 
Weld~\cite{palkar2017weld} is a recent system proposing
...
representation so that application developers can use unified APIs to
call different libraries from Weld.  

\vspace{5pt}
\texttt {Optimized Memory Management.} 
Apache Flink~\cite{alexandrov2014stratosphere},
Apache Spark~\cite{zaharia2012resilient}, 
Spark SQL~\cite{armbrust2015spark} and Deca~\cite{lu2016lifetime} all have
ability to group data into byte arrays or even store data off-heap to
alleviate the garbage collection overhead. 

Flink~\cite{alexandrov2014stratosphere} assigns memory budgets to its data processing operators. Upon
...
SparkSQL~\cite{armbrust2015spark} serializes simple data objects (or
...
further reduce GC overhead. 

Deca~\cite{lu2016lifetime} is a memory management framework aiming at
...
relational databases like MonetDB and Hyper. While PC TCAP shares
similarity in
vectorized processing and code generation techniques with those
systems and can leverage Voodoo's hardware tuning concept, PC is different with those relational processing systems in that all query processing
is based on a high performance object model, and can support efficient
...
addition, it provides a much lower level interfaces compared with PC's
