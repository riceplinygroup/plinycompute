...
efficient distributed computing in a distributed environment called PC
Cluster.  
...
\item The \textbf{PC API and TCAP compiler}.  In the large, PC codes are declarative and look a lot like classical relational calculus \cite{codd1971data}.  For example, to specify a join over five sets of objects, a PC programmer does not build a join DAG over the five inputs, as in a standard
...
\item The \textbf{PC Cluster}, which is a distributed query processing
  system for big data analytics that is based on PC object model, PC
  API and TCAP language. It consists of an optimizer for TCAP
...
future. Among other components such as a catalog server that serves
system metadata, a distributed storage that
contains a buffer pool to buffer writes or cache data in pages, it also consists of a
high-performance distributed, vectorized \emph{TCAP execution engine}. 
The TCAP execution engine is intimately connected to the PC object model, 
...
\vspace{5 pt}
\noindent
\textbf{Roadmap.} We first present an overview of the key components
described in the paper: the PC object model, 
TCAP and vectorized processing, PC's execution engine and so on. Then we describe each part
in more detail. In Sec~\ref{sec:ObjectModel} and Sec~\ref{sec:allocation}, we describe the design,
implementation, and user interfaces of the PC object model, which is
the core and foundation to all other components. In
Sec~\ref{sec:optimizer} and Sec~\ref{sec:pipelined}, we introduce more
details in optimizing and processing TCAP code. In
Sec~\ref{sec:implementation}, we describe the other components in a PC
Cluster, which is the PC distributed computation framework, and how
they work together with PC's distributed execution engine to manipulate PC \texttt{Object}s and process TCAP
to complete distributed computations.