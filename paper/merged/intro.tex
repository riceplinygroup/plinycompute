...
\item The \textbf{execution engine}, which is a distributed query processing
  ...
  processing engine.  
The TCAP execution engine has been designed to work closely with the PC object model to
...
described in the paper: the PC object model, the lambda calculous that
forms the basis of PC API, 
TCAP and PC's execution engin. Then we discuss each part
in more detail. In Section~\ref{sec:ObjectModel}, we describe the design,
implementation, and user interfaces of the PC object model. In
Section~\ref{sec:optimizer} and Section~\ref{sec:pipelined}, we introduce more
details in optimizing and processing TCAP code. In
Section~\ref{sec:implementation}, we describe how all components work
together to execute TCAP and manipulate PC \texttt{Object}s 
for distributed computations such as aggregation and join. In
Section~\ref{sec:experiments}, we present a thorough 
