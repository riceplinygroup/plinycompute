...
(3) the execution engine, and (4) various distributed services. We give an overview of PlinyCompute, focusing on the core components of the
system: the PC object model, 
the PC API and TCAP compiler, and execution engine, and we concretize these ideas by
describing the implementation of a simple $k$-means clustering
algorithm.

\subsection{The PC runtime}
...
\texttt{getSelection (Handle <DataPoint>)} and \texttt{getProjection (Handle <DataPoint>)} which returns a lambda term
...
\subsection{PlinyCompute's Execution Engine}
\label{sec:engine}
...
\subsubsection{Vectorized or Compiled?}
...
the programmer expresses her/his intent at a high-level, by building a graph of high-level computations.  These computations tend to be higher
...
\emph{lambda term
construction functions} as described in Section~\ref{sec:lambda}.  To customize an computation, a user writes one or more member functions
...
PC's C++ binding relies on template metaprogramming (see
Section~\ref{sec:template}) to convert the user-supplied lambda terms
into a TCAP program, where each TCAP operation represents a pipeline
stage over vectors of PC \texttt{Object}s or simple types. 

The TCAP program is then optimized (that is, operations are automatically
re-ordered to form an optimal plan) using classical relational
methods \cite{chaudhuri1998overview, graefe1995cascades,
  jarke1984query}. After optimization, the pipeline stages are fit
together to produce a set of interconnected pipelines.  Input data are broken into lists of 
...
is possible because the programmer expresses intent via the lambda calculus \cite{barendregt1984lambda, moggi1989computational}---we do not rely on inspection of opaque user
...
\subsubsection{TCAP and Vectorized Execution} \label{sec:vectorized}
