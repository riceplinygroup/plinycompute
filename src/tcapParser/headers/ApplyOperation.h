...
namespace pdb_detail
{
    ...
    private:
         ...
         */
        // private because throws exception and PDB style guide disallows exceptions across API boundaries
        ApplyOperation(string applyTarget, ApplyOperationType applyType, TcapIdentifier inputTable,
                       shared_ptr<vector<TcapIdentifier>> inputTableColumnNames, shared_ptr<RetainClause> retain);

        friend shared_ptr<ApplyOperation> makeApplyOperation(TcapTokenStream &tokens); // for constructor
