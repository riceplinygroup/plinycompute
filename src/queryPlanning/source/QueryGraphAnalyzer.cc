...
void QueryGraphAnalyzer::traverse(std::vector<std::string>& tcapStrings,
    ...
    if (numInputs > 0) {
        ...
        for (int i = 0; i < numInputs; i++) {
            ...
            InputTupleSetSpecifier curOutput(
                outputTupleSetName, outputColumnNames, addedOutputColumns);
        ...
        std::cout << "######################" << std :: endl;
        */
        std::string curTcapString = sink->toTCAPString(inputTupleSetsForMe,
                                                       computationLabel,
                                                       outputTupleSetName,
                                                       outputColumnNames,
                                                       addedOutputColumnName);
        // std::cout << computationName << ": " << computationLabel << ": outputTupleSetName: " <<
        // outputTupleSetName << std :: endl;
        // std::cout << computationName << ": " << computationLabel << ":  addedOutputColumns: " <<
        // addedOutputColumnName << std :: endl;
        ...
        if (sink->isTraversed() == false) {
            outputColumnNames.clear();
            addedOutputColumnName = "";
            std::string curTcapString = sink->toTCAPString(inputTupleSets,
                                                           computationLabel,
                                                           outputTupleSetName,
                                                           outputColumnNames,
                                                           addedOutputColumnName);
