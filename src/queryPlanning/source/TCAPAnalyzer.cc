...
TCAPAnalyzer::TCAPAnalyzer(std::string jobId,
    ...
    try {
        ...
        std::cout
            << "FATAL ERROR in TCAPAnalyzer: Not enough memory to allocate the computePlan object";
        logger->fatal(
            "FATAL ERROR in TCAPAnalyzer: Not enough memory to allocate the computePlan object");
        ...
        for (int i = 0; i < this->sources.size(); i++) {
            ...
            if ((sourceComputation->getComputationType() == "ScanUserSet") ||
                ...
                curInputSetIdentifier =
                    makeObject<SetIdentifier>(scanner->getDatabaseName(), scanner->getSetName());
                ...
                curInputSetIdentifier = makeObject<SetIdentifier>(aggregator->getDatabaseName(),
                                                                  aggregator->getSetName());

            } else if (sourceComputation->getComputationType() == "SelectionComp") {
                Handle<SelectionComp<Object, Object>> selector =
                    unsafeCast<SelectionComp<Object, Object>, Computation>(sourceComputation);
                curInputSetIdentifier =
                    makeObject<SetIdentifier>(selector->getDatabaseName(), selector->getSetName());
            } else if (sourceComputation->getComputationType() == "MultiSelectionComp") {
                Handle<MultiSelectionComp<Object, Object>> selector =
                    unsafeCast<MultiSelectionComp<Object, Object>, Computation>(sourceComputation);
                curInputSetIdentifier =
                    makeObject<SetIdentifier>(selector->getDatabaseName(), selector->getSetName());
            } else {
                std::cout << "Source Computation Type: " << sourceComputation->getComputationType()
                          << " are not supported as source node right now" << std::endl;
                this->logger->fatal(
                    "Source Computation Type: " + sourceComputation->getComputationType() +
                    " are not supported as source node right now");
                std::cout << "Master exit...Please restart cluster" << std::endl;
                exit(1);
            }
            std::string mySourceSetName =
                curInputSetIdentifier->getDatabase() + ":" + curInputSetIdentifier->getSetName();
...
// to analyze the subgraph rooted at a source node and only returns a set of job stages
// corresponding with the subgraph
bool TCAPAnalyzer::getNextStages(std::vector<Handle<AbstractJobStage>>& physicalPlanToOutput,
                                 std::vector<Handle<SetIdentifier>>& interGlobalSets,
    ...
    for (int i = 0; i < numConsumersForCurNode; i++) {
        ...
        bool ret = analyze(physicalPlanToOutput,
                           interGlobalSets,
                           tupleSetNames,
                           curSource,
                           sourceComputation,
...
bool TCAPAnalyzer::getNextStagesOptimized(
    ...
    if (curConsumerIndex < consumers.size()) {
        ...
        std::vector<std::string> tupleSetNames;
        tupleSetNames.push_back(outputName);
        ret = analyze(physicalPlanToOutput,
                      interGlobalSets,
                      tupleSetNames,
                      curSource,
                      sourceComputation,
...
// a source computation for a pipeline can be ScanSet, Selection, ClusterAggregation, and
// ClusterJoin.
bool TCAPAnalyzer::analyze(std::vector<Handle<AbstractJobStage>>& physicalPlanToOutput,
                           std::vector<Handle<SetIdentifier>>& interGlobalSets,
                           AtomicComputationPtr curSource,
    ...
    if ((sourceComputation->getComputationType() == "ScanUserSet") ||
        ...
        Handle<AbstractAggregateComp> aggregator =
            unsafeCast<AbstractAggregateComp, Computation>(sourceComputation);
        curInputSetIdentifier =
            makeObject<SetIdentifier>(aggregator->getDatabaseName(), aggregator->getSetName());
        ...
        std::cout << "Source Computation Type: " << sourceComputation->getComputationType()
                  << " are not supported as source node right now" << std::endl;
        this->logger->fatal("Source Computation Type: " + sourceComputation->getComputationType() +
                            " are not supported as source node right now");
        exit(1);
    }
...
Handle<TupleSetJobStage> TCAPAnalyzer::createTupleSetJobStage(
    ...
    jobStageId++;
    jobStage->setComputePlan(
        this->computePlan, sourceTupleSetName, targetTupleSetName, targetComputationName);
    ...
    if ((hashSetsToProbe != nullptr) && (outputForJoinSets.size() > 0) && (isProbing == true)) {
        ...
        for (int i = 0; i < outputForJoinSets.size(); i++) {
            ...
            // std :: cout << "output is " << outputForJoinSets[i] << ", hashSet is " <<
            // (*hashSetToProbeForMe)[outputForJoinSets[i]] << std :: endl;
        }
    ...
    if (sourceContext->isAggregationResult() == true) {
        ...
        jobStage->setNeedsRemoveInputSet(true);  // aggregation output should not be kept across
...
Handle<AggregationJobStage> TCAPAnalyzer::createAggregationJobStage(
    ...
    Handle<AggregationJobStage> aggStage =
        makeObject<AggregationJobStage>(jobStageId, materializeResultOrNot, aggComp);
    jobStageId++;
...
Handle<BroadcastJoinBuildHTJobStage> TCAPAnalyzer::createBroadcastJoinBuildHTJobStage(
    int& jobStageId,
    std::string sourceTupleSetName,
    std::string targetTupleSetName,
    std::string targetComputationName,
    ...
    Handle<BroadcastJoinBuildHTJobStage> broadcastJoinStage =
        makeObject<BroadcastJoinBuildHTJobStage>(this->jobId, jobStageId, hashSetName);
    jobStageId++;
    broadcastJoinStage->setComputePlan(
        this->computePlan, sourceTupleSetName, targetTupleSetName, targetComputationName);
...
Handle<HashPartitionedJoinBuildHTJobStage> TCAPAnalyzer::createHashPartitionedJoinBuildHTJobStage(
    int& jobStageId,
    std::string sourceTupleSetName,
    std::string targetTupleSetName,
    std::string targetComputationName,
    ...
    Handle<HashPartitionedJoinBuildHTJobStage> hashPartitionedJobStage =
        makeObject<HashPartitionedJoinBuildHTJobStage>(this->jobId, jobStageId, hashSetName);
    jobStageId++;
    hashPartitionedJobStage->setComputePlan(
        this->computePlan, sourceTupleSetName, targetTupleSetName, targetComputationName);
...
bool TCAPAnalyzer::updateSourceSets(Handle<SetIdentifier> oldSet,
    ...
    if (numConsumers == numProcessedConsumers) {
        ...
        // std :: cout << "removed for " << sourceSpecifier << " in set " << oldSetName << std ::
        // endl;
    } else {
        // std :: cout << "numConsumers for " << sourceSpecifier << " in set " << oldSetName << " is
...
bool TCAPAnalyzer::analyze(std::vector<Handle<AbstractJobStage>>& physicalPlanToOutput,
    ...
    if (numConsumersForCurNode == 0) {
        ...
        if (myComputation->getComputationType() == "ClusterAggregationComp") {
            ...
            Handle<SetIdentifier> aggregator =
                makeObject<SetIdentifier>(this->jobId, outputName + "_aggregationData");
            ...
            Handle<TupleSetJobStage> jobStage =
                createTupleSetJobStage(jobStageId,
                                       sourceTupleSetName,
                                       curNode->getInputName(),
                                       mySpecifier,
                                       buildTheseTupleSets,
                                       myComputation->getOutputType(),
                                       curInputSetIdentifier,
                                       nullptr,
                                       sink,
                                       false,
            ...
            std::cout << "Sink Computation Type: " << myComputation->getComputationType()
                      << " are not supported as sink node right now" << std::endl;
            this->logger->fatal("Source Computation Type: " + myComputation->getComputationType() +
        ...
        if (curNode->getAtomicComputationType() == "Aggregate") {
            ...
            if (myComputation->needsMaterializeOutput() == true) {
                ...
                aggStage = createAggregationJobStage(
                    jobStageId, agg, aggregator, sink, agg->getOutputType(), true);
            } else {
                sink = makeObject<SetIdentifier>(
                    this->jobId, outputName + "_aggregationResult", PartitionedHashSetType, true);
                aggStage = createAggregationJobStage(
                    jobStageId, agg, aggregator, sink, agg->getOutputType(), false);
            }
            // to push back the aggregation stage;
            physicalPlanToOutput.push_back(aggStage);
            // to push back the aggregator set
            ...
            if (prevNode == nullptr) {
                ...
                // std::cout<<"prev node is null, and target tuple set is " << targetTupleSetName <<
                // std::endl;
            } else {
                targetTupleSetName = prevNode->getOutputName();
                // std::cout<<"prev node is not null, and target tuple set is " <<
                // targetTupleSetName << std::endl;
            }
            ...
            if (joinNode->isTraversed() == false) {
                ...
                if ((isProbing == true) && (this->costOfCurSource <= JOIN_COST_THRESHOLD)) {
                    ...
                    // this is too bad that we we've already probed join tables and join results
                    // could be huge.
                    ...
                    for (j = 0; j < penalizedSourceSets.size(); j++) {
                        // std :: cout << "penalizedSourceSets[" << j << "]=" <<
                        // penalizedSourceSets[j] << std :: endl;
                ...
                if (this->costOfCurSource > JOIN_COST_THRESHOLD) {
                    ...
                    Handle<JoinComp<Object, Object, Object>> join =
                        unsafeCast<JoinComp<Object, Object, Object>, Computation>(myComputation);
                    ...
                    Handle<TupleSetJobStage> joinPrepStage =
                        createTupleSetJobStage(jobStageId,
                                               sourceTupleSetName,
                                               targetTupleSetName,
                                               mySpecifier,
                    ...
                    // std :: cout << "TCAPAnalyzer: outputName = " << outputName << ", hashSetName
                    // = " << hashSetName << std :: endl;
                    Handle<HashPartitionedJoinBuildHTJobStage> joinPartitionStage =
                        createHashPartitionedJoinBuildHTJobStage(jobStageId,
                                                                 sourceTupleSetName,
                                                                 targetTupleSetName,
                                                                 mySpecifier,
                                                                 sink,
                    ...
                    Handle<BroadcastJoinBuildHTJobStage> joinBroadcastStage =
                        createBroadcastJoinBuildHTJobStage(jobStageId,
                                                           sourceTupleSetName,
                                                           targetTupleSetName,
                                                           mySpecifier,
                                                           sink,
                ...
                // We should not go further, we set it to traversed and leave it to other join
                ...
                if (joinNode->isPartitioningLHS() == true) {
                    ...
                    curNode, myComputation, sink, nextNode, jobStageId, curNode, true, myPolicy);
                    }*/
                    ...
                    // std :: cout << "I met a broadcasting join node I have traversed before" <<
                    // std :: endl;
                    buildTheseTupleSets.push_back(curNode->getOutputName());
                    outputForJoinSets.push_back(outputName);
        ...
        // TODO: check whether I am the final operator in a computation. If I am not the final
        ...
        if (myComputation->getComputationType() != "ClusterAggregationComp") {
            ...
            Handle<AbstractAggregateComp> agg =
                unsafeCast<AbstractAggregateComp, Computation>(myComputation);
            Handle<AggregationJobStage> aggStage =
                createAggregationJobStage(jobStageId,
                                          agg,
                                          aggregator,
                                          sink,
                                          agg->getOutputType(),
            ...
            std::cout << "Computation Type: " << myComputation->getComputationType()
                      << " are not supported to have more than one consumers right now"
                      << std::endl;
            this->logger->fatal("Computation Type: " + myComputation->getComputationType() +
                                " are not supported to have more than one consumers right now");
            exit(1);
        }
...
int TCAPAnalyzer::getBestSource(StatisticsPtr stats) {
    ...
    if (stats == 0) {
        ...
        for (int i = 0; i < curSourceSetNames.size(); i++) {
            ...
            // std :: cout << "curSourceSetNames[" << i << "]=" << curSourceSetNames[i] << std ::
            // endl;
        ...
        std::cout << "The Best Source (cost= " << minCost << ") is " << bestIndexToReturn << ": "
...
double TCAPAnalyzer::getCostOfSource(int index, StatisticsPtr stats) {
    ...
    // std :: cout << "curSet has database name=" << curSet->getDatabase() << " and set name=" <<
    // curSet->getSetName() << std :: endl;
