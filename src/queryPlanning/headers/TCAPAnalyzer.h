 ...
 * You can use this class transform a TCAP string into a physical plan: a sequence of
 * AbstractJobStage instances
 * This class must be used in the same allocation block with the invoker of method analyze() to
...
public:
    ...
    // to analyze the subgraph rooted at a source node (sourceComputation) and get a partial phyical
    // plan
    ...
    bool getNextStages(std::vector<Handle<AbstractJobStage>>& physicalPlanToOutput,
                       std::vector<Handle<SetIdentifier>>& interGlobalSets,
    ...
    Handle<HashPartitionedJoinBuildHTJobStage> createHashPartitionedJoinBuildHTJobStage(
        int& jobStageId,
        std::string sourceTupleSetName,
        std::string targetTupleSetName,
        std::string targetComputationName,
    ...
    bool analyze(std::vector<Handle<AbstractJobStage>>& physicalPlanToOutput,
                 ...
                 int& jobStageId,
                 AtomicComputationPtr prevComputation = nullptr,
                 bool isProbing = false,
                 AllocatorPolicy policy = defaultAllocator,
...
private:
    ...
    // the mapping of set name to source set identifiers, which will only be used in dynamicPlanning
    // mode, and will keep changing
    std::map<std::string, Handle<SetIdentifier>> curSourceSets;

    // the mapping of set name to source computation node, which will only be used in
