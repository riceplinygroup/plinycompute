...
//            cout << "first: " << nodeIP.c_str() << " push_back set: " << setName.c_str();
    ...
    void CatalogStandardDatabaseMetadata :: removeNodeFromSet(string node, string set) {
        deleteNodeFromSingleSet(node, set);
        deleteSetFromSingleNode(set, node);
    }

    void CatalogStandardDatabaseMetadata :: deleteNodeFromMap(string &nodeIP, string &setName){
...
//                output.append("\n -Set: ").append(item.first.c_str()).append(" is stored in (").append(to_string(item.second.size())).append(") Nodes: [ ");
//                for (int i=0; i < item.second.size(); i++){
//                    if (i>0) output.append(", ").append(spaces).append(item.second[i].c_str());
...
//            output.append(" ]");
        ...
        for (auto &item : nodesInDB){
            if (i>0) output.append(", ").append(spaces).append(item.first);
            else output.append(item.first);
            i++;
        }
        ...
        for (auto &item : setsInDB){
            if (i>0) output.append(", ").append(spaces).append(item.first.c_str());
            else output.append(item.first.c_str());
            i++;
        }
        output.append(" ]");
