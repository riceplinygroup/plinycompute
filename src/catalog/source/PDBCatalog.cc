...
bool PDBCatalog::addItemToVector(Handle<CatalogUserTypeMetadata>& item, int& key) {
    ...
    mapTypeNameToTypeID.insert(
        make_pair(item->getItemName().c_str(), atoi(item->getObjectID().c_str())));
    mapTypeIdToTypeName.insert(
        make_pair(atoi(item->getObjectID().c_str()), item->getItemName().c_str()));
    return true;
}

/* Specialization for updating Database metadata */
template <>
bool PDBCatalog::updateItemInVector(int& index, Handle<CatalogDatabaseMetadata>& item) {
    auto begin = std::chrono::high_resolution_clock::now();
...
template <>
bool PDBCatalog::updateItemInVector(int& index, Handle<CatalogSetMetadata>& item) {
    auto begin = std::chrono::high_resolution_clock::now();
...
template <>
bool PDBCatalog::updateItemInVector(int& index, Handle<CatalogNodeMetadata>& item) {
...
template <>
bool PDBCatalog::updateItemInVector(int& index, Handle<CatalogUserTypeMetadata>& item) {
...
template <>
bool PDBCatalog::deleteItemInVector(int& index, Handle<CatalogSetMetadata>& item) {
...
template <>
bool PDBCatalog::deleteItemInVector(int& index, Handle<CatalogNodeMetadata>& item) {
...
template <>
bool PDBCatalog::deleteItemInVector(int& index, Handle<CatalogUserTypeMetadata>& item) {
...
PDBCatalog::PDBCatalog(PDBLoggerPtr logger, string location) {
    ...
    if (mkdir(catalogRootPath.c_str(), S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH) == -1) {
        ...
        PDB_COUT << "Parent catalog folder " << catalogRootPath << "  was created/opened."
                 << std::endl;
    }

    // Creates a location folder for storing the sqlite file containing metadata for this PDB
    // instance.
    // If location exists, only opens it.
    if (mkdir(catalogPath.c_str(), S_IRWXU | S_IRWXG | S_IROTH | S_IXOTH) == -1) {
        PDB_COUT << "Catalog folder " << catalogPath << " was not created, it already exists."
                 << std::endl;
    } else {
    ...
    mapsPDBOjbect2SQLiteTable.insert(make_pair(PDBCatalogMsgType::CatalogPDBNode, "pdb_node"));
    mapsPDBOjbect2SQLiteTable.insert(
        make_pair(PDBCatalogMsgType::CatalogPDBDatabase, "pdb_database"));
    mapsPDBOjbect2SQLiteTable.insert(make_pair(PDBCatalogMsgType::CatalogPDBSet, "pdb_set"));
    mapsPDBOjbect2SQLiteTable.insert(make_pair(PDBCatalogMsgType::CatalogPDBUser, "pdb_user"));
    mapsPDBOjbect2SQLiteTable.insert(
        make_pair(PDBCatalogMsgType::CatalogPDBPermissions, "pdb_user_permission"));
    mapsPDBOjbect2SQLiteTable.insert(
    ...
    // in order to create query strings
    mapsPDBArrayOjbect2SQLiteTable.insert(make_pair(PDBCatalogMsgType::CatalogPDBNode, "pdb_node"));
    mapsPDBArrayOjbect2SQLiteTable.insert(
        make_pair(PDBCatalogMsgType::CatalogPDBDatabase, "pdb_database"));
    mapsPDBArrayOjbect2SQLiteTable.insert(make_pair(PDBCatalogMsgType::CatalogPDBSet, "pdb_set"));
    mapsPDBArrayOjbect2SQLiteTable.insert(make_pair(PDBCatalogMsgType::CatalogPDBUser, "pdb_user"));
    mapsPDBArrayOjbect2SQLiteTable.insert(
        make_pair(PDBCatalogMsgType::CatalogPDBPermissions, "pdb_user_permission"));
    mapsPDBArrayOjbect2SQLiteTable.insert(
        make_pair(PDBCatalogMsgType::CatalogPDBRegisteredObject, "data_types"));
...
void PDBCatalog::open() {
    ...
    if ((ret = sqlite3_open_v2(uriPath.c_str(),
                               &sqliteDBHandler,
        ...
        // Create tables if they don't exist, they are created with primary key to prevent
        // duplicates
        catalogSqlQuery(
        ...
        catalogSqlQuery(
            ...
            "itemInfo BLOB, timeStamp INTEGER);");
        catalogSqlQuery(
            "CREATE TABLE IF NOT EXISTS pdb_user_permission (itemID TEXT PRIMARY KEY, "
            "itemInfo BLOB, timeStamp INTEGER);");
    ...
    PDB_COUT << "Time Duration for open SQLite:\t "
             << std::to_string(
                    std::chrono::duration_cast<std::chrono::duration<float>>(query - begin).count())
             << " secs." << endl;
    PDB_COUT << "Time Duration for sqlite CREATES/OPENS:\t "
             << std::to_string(
                    std::chrono::duration_cast<std::chrono::duration<float>>(load - query).count())
             << " secs." << endl;
    PDB_COUT << "Time Duration for loads in memory includes selects in SQLite:\t "
             << std::to_string(
                    std::chrono::duration_cast<std::chrono::duration<float>>(end - load).count())
             << " secs." << endl;
...
void PDBCatalog::loadsMetadataIntoMemory() {
    ...
    if (getMetadataFromCatalog(false,
        ...
        // populates maps
        for (int i = 0; i < (*registeredDatabasesMetadata).size(); i++) {
            this->logger->debug("RETRIEVING db " +
        ...
        for (int i = 0; i < (*registeredSetsMetadata).size(); i++) {
            ...
            registeredSets.insert(make_pair((*registeredSetsMetadata)[i].getItemKey().c_str(),
        ...
        for (int i = 0; i < (*registeredNodesMetadata).size(); i++) {
            ...
            registeredNodes.insert(make_pair((*registeredNodesMetadata)[i].getItemKey().c_str(),
                                             (*registeredNodesMetadata)[i]));
        }
        for (int i = 0; i < (*registeredUserDefinedTypesMetadata).size(); i++) {
            this->logger->debug("RETRIEVING TYPE " +
            ...
            registeredUserDefinedTypes.insert(
                make_pair((*registeredUserDefinedTypesMetadata)[i].getItemName().c_str(),
...
void PDBCatalog::getModifiedMetadata(Handle<CatalogPrintMetadata> &itemMetadata) {
    ...
    cout << "\nNodes in cluster: " + std::to_string((int)(*_registeredNodesMetadata).size())
         << endl;
    cout << "----------------------------" << endl;
    ...
    cout << "\nDatabases: " + std::to_string((int)(*_registeredDatabasesMetadata).size()) << endl;
    cout << "----------------------------" << endl;

    for (int i = 0; i < (*_registeredDatabasesMetadata).size(); i++) {
        cout << (*_registeredDatabasesMetadata)[i].printShort() << endl;
    }
    cout << "\nSets: " + std::to_string((int)(*_registeredSetsMetadata).size()) << endl;
    cout << "----------------------------" << endl;
...
void PDBCatalog::printsAllCatalogMetadata() {
    ...
    cout << "\nI. Nodes in the cluster (" + std::to_string((int)(*registeredNodesMetadata).size())
         << ")" << endl;
    cout << "----------------------------" << endl;
    ...
    cout << "\nII. Databases (" + std::to_string((int)(*registeredDatabasesMetadata).size()) << ")"
         << endl;
    cout << "----------------------------" << endl;
    ...
    cout << "\nIII. Sets (" + std::to_string((int)(*registeredSetsMetadata).size()) << ")" << endl;
    cout << "----------------------------" << endl;
...
bool PDBCatalog::getMetadataFromCatalog(bool onlyModified,
    ...
    if (sqlite3_prepare_v2(sqliteDBHandler, queryString.c_str(), -1, &statement, NULL) ==
        SQLITE_OK) {
        ...
        while (1) {
            ...
            if (res == SQLITE_ROW) {
                ...
                PDB_COUT << "entry " << sqlite3_column_text(statement, 0) << " timestamp "
...
bool PDBCatalog::registerUserDefinedObject(int16_t typeCode,
    ...
    if (rc != SQLITE_OK) {
        ...
        if (rc != SQLITE_OK) {
            errorMessage =
                "Bind operation failed. " + (string)sqlite3_errmsg(sqliteDBHandler) + "\n";
            this->logger->debug(errorMessage);
            success = false;
        } else {
            ...
            if (rc != SQLITE_DONE) {
                if (sqlite3_errcode(sqliteDBHandler) == SQLITE_CONSTRAINT) {
                    ...
                    errorMessage =
                        "Query execution failed. " + (string)sqlite3_errmsg(sqliteDBHandler) + "\n";
            } else {
...
bool PDBCatalog::retrievesDynamicLibrary(string itemId,
    ...
    if (sqlite3_prepare_v2(sqliteDBHandler, query.c_str(), -1, &pStmt, NULL) != SQLITE_OK) {
        errorMessage =
            "Error query not well formed: " + (string)sqlite3_errmsg(sqliteDBHandler) + "\n";
    ...
    if (sqlite3_step(pStmt) != SQLITE_ROW) {
        errorMessage =
            "Error item not found in database: " + (string)sqlite3_errmsg(sqliteDBHandler) + "\n";
        sqlite3_reset(pStmt);
        PDB_COUT << errorMessage << endl;
...
bool PDBCatalog::addMetadataToCatalog(pdb::Handle<CatalogMetadataType>& metadataValue,
    ...
    if (isSuccess == true) {
        this->logger->debug(
            "The following item metadata was stored in SQLite and loaded into Catalog memory:");
    ...
    this->logger->debug(
        ...
        std::to_string(
            std::chrono::duration_cast<std::chrono::duration<float>>(end - realSQLInsert).count()) +
        " secs.");
    return isSuccess;
}
...
bool PDBCatalog::deleteMetadataInCatalog(pdb::Handle<CatalogMetadataType> metadataValue,
    ...
    sqlite3_stmt* stmt = NULL;
    string sqlStatement =
        "DELETE from " + mapsPDBOjbect2SQLiteTable[metadataCategory] + " where itemId = ?";
...
std::ostream& operator<<(std::ostream& out, PDBCatalog& catalog) {
    ...
    for (int i = 0; i < catalog.registeredSetsMetadata->size(); i++) {
        out << "      Id: " << (*catalog.registeredSetsMetadata)[i].getItemId().c_str()
            ...
            << " | Set: " << (*catalog.registeredSetsMetadata)[i].getItemName().c_str() << endl;
    }
    ...
    out << "\n   Number of user-defined types registered: " +
            std::to_string((int)catalog.registeredUserDefinedTypesMetadata->size())
        << endl;
    for (int i = 0; i < catalog.registeredUserDefinedTypesMetadata->size(); i++) {
        out << "      Id: " << (*catalog.registeredUserDefinedTypesMetadata)[i].getItemId().c_str()
            << " | Type Name: "
            << (*catalog.registeredUserDefinedTypesMetadata)[i].getItemName().c_str() << endl;
    }
    out << "--------------------------" << endl;
    return out;
}
...
template bool PDBCatalog::updateMetadataInCatalog(
    pdb::Handle<CatalogDatabaseMetadata>& metadataValue, int& catalogType, string& errorMessage);

/* Explicit instantiation for updating User-defined Type Metadata in the catalog */
template bool PDBCatalog::updateMetadataInCatalog(
    pdb::Handle<CatalogUserTypeMetadata>& metadataValue, int& catalogType, string& errorMessage);
