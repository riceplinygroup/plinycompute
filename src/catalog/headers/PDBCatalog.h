...
public:
   ...
   * saving it
   * as serialized bytes in an Sqlite table, it then adds that metadata item
   * into memory.
   *
  ...
  /**
   * addItemToVector loads a registered Metadata item into memory so it can be
   * accessed by
   * the CatalogServer
   *
  ...
  /**
   * updateMetadataInCatalog updates an existing Metadata item in Sqlite along
   * with its
   * content in memory
   *
   * @param metadataValue encapsulates the object with the metadata
  ...
  /**
   * deleteMetadataInCatalog deletes an existing Metadata item in Sqlite along
   * with its
   * content in memory
   *
   * @param metadataValue encapsulates the object with the metadata
  ...
  /**
   * updateItemInVector updates a registered Metadata item in memory so the
   * changes
  ...
  /**
   * deleteItemInVector deletes a registered Metadata item in memory so the
   * changes
   * are visible to the CatalogServer
   *
   * @param index is the position in the container
  ...
  /**
   * getSerializedCatalog retrieves the bytes of the entire catalog, this could
   * be used
  ...
  /**
   * setCatalogVersion sets the version of the catalog, this is typically called
   * when
  ...
  /**
   * getMetadataFromCatalog retrieves all Metadata from Sqlite for a given
   * category and
   ...
   * is passed
   *        as: 1388534400000) and will return all entries created after that
   * given timestamp
  ...
  /**
   * getLastId gets the number of items in a given Metadata category, so the id
   * can be
   ...
   */
  bool retrievesDynamicLibrary(string fileName, string tableName,
                               Handle<CatalogUserTypeMetadata> &returnedItem,
                               string &returnedSoLibrary, string &errorName);

  /**
   * getListOfDatabases returns a Vector of metadata for all Databases
   * registered
   * in the catalog. The information is retrieved from memory rather than disk
   * (SQLite).
   *
   * @param key to search for
   *
   ...
   * @return a Vector of items
   */
  void getListOfSets(Handle<Vector<CatalogSetMetadata>> &setsInCatalog,
                     const string &key);
...
private:
  /**
   * getMapsPDBOjbect2SQLiteTable returns the name of a Sqlite table for a given
   category of
   * metadata
  ...
  /**
   * loadsMetadataIntoMemory loads all Metdata stored in Sqlite and populates
   * the corresponding
   * containers
   */
  ...
  /**
   * Contains information of users registered in the catalog. (To be
   *implemented)
   **/
   ...
   * Maps the name of an SQLite table given the type of metadata it contains.
   *Used for
   * composing prepared SQL statements in SQLite, given a tyep of Metadata in a
   *container
   **/
