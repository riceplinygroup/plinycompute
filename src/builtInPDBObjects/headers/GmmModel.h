...
public:
    ...
    GmmModel(int k, int ndim) {
        ...
        // pdb::Handle<pdb::Vector<Handle<DoubleVector>>> my_model =
        // pdb::makeObject<pdb::Vector<Handle<DoubleVector>>> ();
    ...
    double log_normpdf(int i, Handle<DoubleVector> inputData, bool isLog) {
        ...
        // Function: int gsl_blas_dsymv (CBLAS_UPLO_t Uplo, double alpha, const gsl_matrix * A,
        // const gsl_vector * x, double beta, gsl_vector * y)

        // These functions compute the matrix-vector product and sum y = \alpha A x + \beta y for
        ...
        /*Eigen::Map<Eigen::Matrix<double,Eigen::Dynamic,Eigen::Dynamic,Eigen::RowMajor> > my_inv
        ((*covars)[i]->data->c_ptr(),ndim,ndim);
    ...
    /*double log_normpdf(int i, Handle<DoubleVector> inputData, bool isLog) {
        ...
        gsl_vector_view mean = gsl_vector_view_array((*means)[i]->data->c_ptr(), ndim);
        gsl_matrix_view covar = gsl_matrix_view_array((*covars)[i]->data->c_ptr(), ndim, ndim);
        gsl_matrix_view inv_covar = gsl_matrix_view_array((*inv_covars)[i]->data->c_ptr(), ndim,
    ndim);
        ...
        //Function: int gsl_blas_dsymv (CBLAS_UPLO_t Uplo, double alpha, const gsl_matrix * A, const
    gsl_vector * x, double beta, gsl_vector * y)

        //These functions compute the matrix-vector product and sum y = \alpha A x + \beta y for the
    symmetric matrix A
        gsl_blas_dsymv(CblasUpper, 1.0, &inv_covar.matrix, datan, 0.0, ym);
    ...
    void updateModel(GmmNewComp update) {
        ...
        for (int i = 0; i < k; i++) {
            ...
            gsl_vector_view mean = gsl_vector_view_array((*means)[i]->data->c_ptr(), ndim);
            gsl_vector_view gweightedX =
                gsl_vector_view_array(update.getWeightedX(i).data->c_ptr(), ndim);
            ...
            gsl_vector_view gweightedX2v =
                gsl_vector_view_array(update.getWeightedX2(i).data->c_ptr(), ndim * ndim);
            gsl_vector_scale(&gweightedX2v.vector, 1.0 / sumR[i]);

            // std::cout << "gweightedX2 B" << i << std::endl; update.getWeightedX2(i).print();

            gsl_matrix_view gweightedX2m =
                gsl_matrix_view_array(update.getWeightedX2(i).data->c_ptr(), ndim, ndim);
    ...
    Handle<GmmNewComp> createNewComp(Handle<DoubleVector>& data) {
        ...
        Handle<Vector<DoubleVector>> newweightedX =
            makeObject<Vector<DoubleVector>>();  // Sum of r*x
        Handle<Vector<DoubleVector>> newweightedX2 =
            makeObject<Vector<DoubleVector>>();  // Sum of r*(x**2)
        ...
        for (int i = 0; i < k; i++) {
            ...
            gsl_matrix_view gweightedX2 =
                gsl_matrix_view_array(weightedX2->data->c_ptr(), ndim, ndim);
            ...
            // gsl_blas_dsyr (CBLAS_UPLO_t Uplo, double alpha, const gsl_vector * x, gsl_matrix * A)
            // - //A = \alpha x x^T + A
        ...
        Handle<GmmNewComp> result =
            makeObject<GmmNewComp>(logLikelihood, *r_values, *newweightedX, *newweightedX2);
