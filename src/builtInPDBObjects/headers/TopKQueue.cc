...
void TopKQueue <Score, ValueType> :: setUpAndCopyFrom (void *target, void *source) const {
	...
	if (!to->scoreTypeInfo.descendsFromObject ()) { 
		memmove ((void *) &(to->tempScore), (void *) &(from->tempScore), to->scoreTypeInfo.getSizeOfConstituentObject (nullptr));
	} else {
	...
	if (!to->valueTypeInfo.descendsFromObject ()) { 
		memmove (from->valueOffset + (char *) to, from->valueOffset + (char *) from, to->valueTypeInfo.getSizeOfConstituentObject (nullptr));
	} else {
		to->valueTypeInfo.setUpAndCopyFromConstituentObject (from->valueOffset + (char *) to, from->valueOffset + (char *) from);
	}
...
template <class Score, class ValueType>
TopKQueue <Score, ValueType> &TopKQueue <Score, ValueType> ::operator + (TopKQueue <Score, ValueType> &addMeIn) {
