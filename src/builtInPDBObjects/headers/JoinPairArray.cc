...
void JoinPairArray <ValueType> :: setUpAndCopyFrom (void *target, void *source) const {
	...
	for (int i = 0; i < toMe.numSlots; i++) {
		...
		if (!toMe.valueTypeInfo.descendsFromObject ()) {
			...
			toMe.valueTypeInfo.setUpAndCopyFromConstituentObject (JM_GET_VALUE_PTR (toMe.data, i), JM_GET_VALUE_PTR (fromMe.data, i));
		}
	}
}
...
void JoinPairArray <ValueType> :: setUnused (const size_t &me) {
	...
	size_t hashVal = me == JM_UNUSED ? 858931273 : me;
        ...
        for (size_t slotsChecked = 0; slotsChecked < numSlots; slotsChecked++) {
			...
			if (JM_GET_NEXT (data, slot) != UINT32_MAX && overflows[JM_GET_NEXT (data, slot)].size () >= 1) {
...
Handle <JoinPairArray <ValueType>> JoinPairArray <ValueType> :: doubleArray () {
        PDB_COUT << "bytes available in current allocator block: " << getAllocator().getBytesAvailableInCurrentAllocatorBlock () << std :: endl;
...
template <class ValueType>
JoinRecordList <ValueType> :: JoinRecordList (uint32_t whichOne, JoinPairArray <ValueType> &parent) : whichOne (whichOne), parent (parent) {}
...
size_t JoinRecordList <ValueType> :: size () {
	...
	return JM_GET_NEXT (parent.data, whichOne) == UINT32_MAX ? 1 : parent.overflows[JM_GET_NEXT (parent.data, whichOne)].size () + 1;
}
