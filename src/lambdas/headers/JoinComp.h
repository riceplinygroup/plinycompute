...
public:
    ...
    JoinArg(ComputePlan& plan, void* pageWhereHashTableIs)
        : plan(plan), pageWhereHashTableIs(pageWhereHashTableIs) {}
...
public:
    ...
    SinkMergerPtr getSinkMerger(TupleSpec& consumeMe,
        ...
        for (auto& a : projection.getAtts()) {
            ...
            // to ask for
            if (res.second == "") {
                // std :: cout << "my type is " << plan.getPlan ()->getNode
                // (res.first).getComputation ().getOutputType () << std :: endl;
                typeList.push_back(
                    "pdb::Handle<" +
    ...
    virtual std::string toTCAPString(std::vector<InputTupleSetSpecifier>& inputTupleSets,
        ...
        // std :: cout << "to get TCAPString for Computation: JoinComp_" << computationLabel << "
        // with " << inputTupleSets.size() << "inputs" << std :: endl;
        if (inputTupleSets.size() == getNumInputs()) {
            ...
            for (unsigned int i = 0; i < inputTupleSets.size(); i++) {
                ...
                // std :: cout << i << "-th input tuple set name is " <<
                // inputTupleSets[i].getTupleSetName() << std :: endl;
                ...
                for (unsigned int j = 0; j < inputTupleSets[i].getColumnNamesToKeep().size(); j++) {
                    // std :: cout << i << ": " << j << "-th column to keep is " <<
                    // (inputTupleSets[i].getColumnNamesToKeep())[j] << std :: endl;
                }
                ...
                // std :: cout << i << "-th column to apply is " <<
                // inputTupleSets[i].getColumnNamesToApply()[0] << std :: endl;
            }
            ...
            // std :: cout << "outputColumnNames[" << i << "]=" << outputColumnNames[i] << std ::
            // endl;
            //}
    ...
    ComputeExecutorPtr getExecutor(bool needToSwapAtts,
        ...
        for (auto& a : (hashedInputSchema.getAtts())) {
            ...
            // find the identity of the producing computation
            // std :: cout << "finding the source of " << hashedInputSchema.getSetName () << "." <<
            // a << "\n";
            std::pair<std::string, std::string> res =
