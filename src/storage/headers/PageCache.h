...
struct CacheKeyEqual {
    ...
    bool operator()(const CacheKey& lKey, const CacheKey& rKey) const {
        if ((lKey.dbId == rKey.dbId)&&(lKey.typeId == rKey.typeId)&&(lKey.setId == rKey.setId)&&(lKey.pageId == rKey.pageId)) {
            return true;
        } else {
            return false;
        }
...
/**
 * This class wraps a global page cache adopting multiple eviction policy, and by default it uses MRU.
 ...
 * Step 2. For each loaded page, frontend will pin it, and tell backend about the location of the pinned page
 * Step 3. Backend will tell frontend to unpin a page
 *
 * Typical scenario for output data
 * Step 1. Backend will tell frontend to create a temp set or a user set, depending on output data's lifetime and visibility;
 ...
 * Step 4. Backend will tell frontend to pin the page again;(at this time, page may be flushed to disk file, and needs to load it to cache first)
...
public:
    ...
    //load it to cache. Otherwise, if the page is not in cache, and can not be found in any flushed file,
    //we return nullptr.
    ...
    //Otherwise, if the page is not in cache, and can not be found in any flushed file, we return nullptr.
    //Below method will cause page reference count ++
    //This function can be used for frontend to pin a flushed page.
    //This function can also be used for backend to pin a flushed page that has been pinned and unpinned before by the same backend.
    ...
    //If sequential=true, we will invoke file's sequential read API if the file instance has provided such API.
    ...
    //Allocate buffer of required size from shared memory, if no room, block and run eviction thread.
    char * allocateBufferFromSharedMemoryBlocking(size_t size, int & alignOffset);

    //TODO: Allocate buffer of required size from shared memory, if no room, block and evict only one page.
