...
//JiaNote: this version of data generator cleanup allocation blocks manually to resolve the memory leak issue brought by cross-block allocation.
...
int main(int argc, char * argv[]) {
	...
	string errMsg;
        ...
        for (int i = 0; i < numFullRounds; i ++) {
	        ...
	        // Generate the data
	        dataGenerator(scaleFactor, dispatcherClient, noOfCopiesEachRound);
	        // flush to disk
        ...
        if (noOfCopiesPartialRound > 0) { 
            {
                pdb::makeObjectAllocatorBlock((size_t) 2 * GB, true);
                // Generate the data
                dataGenerator(scaleFactor, dispatcherClient, noOfCopiesPartialRound);
                // flush to disk
                distributedStorageManagerClient.flushData(errMsg);
                cout << errMsg << endl;
            }
