...
void testBuildLogicalPlanFromApplyFunction(UnitTest& qunit) {
    ...
    ApplyFunctionPtr applyFunction;
    {
        ...
        applyFunction = makeApplyFunction(
            "exec", "someFunctionName", "outputTable", "outputColumn", inputCols, copyCols);
    }
    ...
    buildLogicalPlan(ops)->apply(
        [&](LogicalPlan logicalPlan) {
            ...
            ComputationPtr computation =
                logicalPlan.getComputations().getProducingComputation("outputTable");
...
void testBuildLogicalPlanFromApplyMethod(UnitTest& qunit) {
    ...
    ApplyMethodPtr applyMethod;
    {
        ...
        applyMethod = makeApplyMethod(
            "exec", "someMethodName", "outputTable", "outputColumn", inputCols, copyCols);
    }
...
void testBuildLogicalPlanFromHoist(UnitTest& qunit) {
    ...
    HoistPtr hoist;
    {
        ...
        hoist = makeHoist("fieldId",
                          TableColumn("inputTable", "InputColumn"),
                          TableColumn("outputTable", "outputColumn"),
                          copyCols,
                          "exec");
    }
...
void testBuildLogicalPlanFromGreaterThan(UnitTest& qunit) {
    ...
    GreaterThanPtr greaterThan;
    {
        ...
        greaterThan = makeGreaterThan(TableColumn("A", "1"),
                                      TableColumn("A", "2"),
                                      TableColumn("outputTable", "isGreater"),
                                      copyCols,
                                      "exec");
    }
...
void testBuildLogicalPlanFromStore(UnitTest& qunit) {
    ...
    buildLogicalPlan(ops)->apply(
        [&](LogicalPlan logicalPlan) {
            ...
            QUNIT_IS_EQUAL("inputTable",
                           outputs.getConsumers("inputTable").operator[](0).getInputName());
            QUNIT_IS_EQUAL(
                2, outputs.getConsumers("inputTable").operator[](0).getInput().getAtts().size());
            QUNIT_IS_EQUAL(
                "1", outputs.getConsumers("inputTable").operator[](0).getInput().getAtts()[0]);
            QUNIT_IS_EQUAL(
                "2", outputs.getConsumers("inputTable").operator[](0).getInput().getAtts()[1]);
        },
        [&](string errorMsg) { QUNIT_IS_TRUE(false); });
}
...
void testBuildLogicalPlanFromStringTest47(UnitTest& qunit) {
    ...
    logicalPlanResult->apply(
        ...
        [&](string error) { QUNIT_IS_TRUE(false); });
}
}
