...
void CatalogServer::registerHandlers(PDBServer& forMe) {
    ...
    forMe.registerHandler(
        ...
        make_shared<SimpleRequestHandler<CatalogDatabaseMetadata>>(
            [&](Handle<CatalogDatabaseMetadata> request, PDBCommunicatorPtr sendUsingMe) {
                ...
                PDB_COUT << "--->CatalogServer handler CatalogDatabaseMetadata_TYPEID calling "
                            "addDatabaseMetadata"
                         << endl;
                const UseTemporaryAllocationBlock block{1024 * 1024};
                bool res = false;
    ...
    forMe.registerHandler(
        ...
        make_shared<SimpleRequestHandler<CatalogSetMetadata>>([&](
            ...
            PDB_COUT << "--->CatalogServer handler CatalogSetMetadata_TYPEID calling addSetMetadata"
                     << endl;
            const UseTemporaryAllocationBlock block{1024 * 1024};
    ...
    // handles a request to display the contents of the Catalog that have changed since a given
    // timestamp
    forMe.registerHandler(
    ...
    forMe.registerHandler(
        ...
        make_shared<SimpleRequestHandler<CatTypeNameSearch>>(
            [&](Handle<CatTypeNameSearch> request, PDBCommunicatorPtr sendUsingMe) {
                ...
                // in practice, we can do better than simply locking the whole catalog, but good
                // enough for now...
    ...
    forMe.registerHandler(
        ...
        make_shared<SimpleRequestHandler<CatSharedLibraryRequest>>(
            [&](Handle<CatSharedLibraryRequest> request, PDBCommunicatorPtr sendUsingMe) {
                ...
                PDB_COUT << "CatalogServer to handle CatSharedLibraryRequest to get shared library "
                            "for typeID="
                ...
                if (!res) {
                    ...
                    Handle<Vector<char>> response =
                        makeObject<Vector<char>>((*putResultHere).size(), (*putResultHere).size());
    ...
    forMe.registerHandler(
        ...
        make_shared<SimpleRequestHandler<CatSharedLibraryByNameRequest>>([&](
            ...
            int16_t typeId = 0;
            // if typeName is empty we are searching by typeId, hence retrieves the typeId from the
            // request object
            if (typeName.empty() == true) {
                typeId = request->getTypeLibraryId();
            } else {
                // if a typeName is provided, we are searching by that name, so first we have to
                // retrieve
                // the typeID, given the provided typeName
                typeId = allTypeNames[typeName];
            }

            // in practice, we can do better than simply locking the whole catalog, but good enough
            // for now...
            const LockGuard guard{workingMutex};
            PDB_COUT
                << "Triggering Handler CatalogServer CatSharedLibraryByNameRequest for typeName="
                << typeName << " and typeId=" << std::to_string(typeId) << endl;
            ...
            PDB_COUT << std::string(
                            "CatalogServer to handle CatSharedLibraryByNameRequest to get shared "
            ...
            if (this->isMasterCatalogServer == true) {
                ...
                PDB_COUT << "    Invoking getSharedLibrary(typeName) from CatalogServer Handler "
                            "b/c this is Master Catalog "
                         << endl;

                // if the type is not registered in the Master Catalog just return with a typeID =
                // -1
                if (allTypeCodes.count(typeId) == 0) {
                    ...
                    // the type was found in the catalog, retrieves metadata and bytes of the Shared
                    // Library
                    ...
                    // copies the bytes of the Shared Library to the object to be sent back to the
                    // caller
                ...
                if (allTypeCodes.count(typeId) == 0) {
                    ...
                    PDB_COUT << "    Connecting to the Remote Catalog Server via Catalog Client"
                             << endl;
                    PDB_COUT << "    Invoking CatalogClient.getSharedLibraryByName(typeName) from "
                    ...
                    // in the local catalog, given the library and metadata retrieved from the
    ...
    forMe.registerHandler(
        ...
        make_shared<SimpleRequestHandler<CatSetObjectTypeRequest>>([&](
            ...
            PDB_COUT << "typeID for Set with dbName=" << string(request->getDatabaseName())
                     << " and setName=" << string(request->getSetName()) << " is "
                     << std::to_string(typeID) << endl;
            ...
            if (typeID >= 0)
                ...
                response =
                    makeObject<CatTypeNameSearchResult>("", false, "could not find requested type");
    ...
    forMe.registerHandler(CatCreateSetRequest_TYPEID,
    ...
    forMe.registerHandler(
        ...
        make_shared<SimpleRequestHandler<CatDeleteDatabaseRequest>>(
            [&](Handle<CatDeleteDatabaseRequest> request, PDBCommunicatorPtr sendUsingMe) {
                ...
                bool res =
                    getFunctionality<CatalogServer>().deleteDatabase(request->dbToDelete(), errMsg);
        ...
        make_shared<SimpleRequestHandler<CatAddNodeToSetRequest>>(
            [&](Handle<CatAddNodeToSetRequest> request, PDBCommunicatorPtr sendUsingMe) {
                ...
                bool res = getFunctionality<CatalogServer>().addNodeToSet(
                    request->nodeToAdd(), request->whichDB(), request->whichSet(), errMsg);
        ...
        make_shared<SimpleRequestHandler<CatRemoveNodeFromSetRequest>>(
            [&](Handle<CatRemoveNodeFromSetRequest> request, PDBCommunicatorPtr sendUsingMe) {
                ...
                bool res = getFunctionality<CatalogServer>().removeNodeFromSet(
                    request->nodeToRemove(), request->whichDB(), request->whichSet(), errMsg);
    ...
    forMe.registerHandler(CatRegisterType_TYPEID,
...
std::string CatalogServer::searchForObjectTypeName(int16_t typeIdentifier) {
    ...
    PDB_COUT << "searchForObjectTypeName with typeIdentifier =" << std::to_string(typeIdentifier)
             << endl;
...
bool CatalogServer::getSharedLibraryByTypeName(std::string typeName,
    ...
    if (pdbCatalog->retrievesDynamicLibrary(
            typeName, typeOfObject, itemMetadata, returnedBytes, errMsg) == true) {
...
int16_t CatalogServer::addObjectType(int16_t typeIDFromMasterCatalog,
    ...
    if ((dlsym_error = dlerror())) {
        errMsg = "Error, can't load function getObjectTypeName in the shared library. " +
            string(dlsym_error) + '\n';
        PDB_COUT << errMsg << endl;
        return -1;
    }
    ...
    if (allTypeNames.count(typeName) == 0) {
        ...
        pdbCatalog->registerUserDefinedObject(typeCode,
                                              objectMetadata,
                                              std::string(soFile.begin(), soFile.end()),
                                              typeName,
                                              catalogDirectory + "/tmp_so_files/" + typeName +
                                                  ".so",
                                              "data_types",
                                              errMsg);
...
bool CatalogServer::deleteSet(std::string databaseName, std::string setName, std::string& errMsg) {
    ...
    if (pdbCatalog->getMetadataFromCatalog(
            false, databaseName, databaseItems, errMsg, catalogType) == false)
        PDB_COUT << errMsg << endl;
    ...
    if (isMasterCatalogServer) {
        ...
        for (auto& item : updateResults) {
            PDB_COUT << "Set Metadata in node IP: " << item.first
                     << ((item.second.first == true) ? " updated correctly!"
        ...
        PDB_COUT << "This is not Master Catalog Node, thus metadata was only registered locally!"
                 << endl;
    }
...
bool CatalogServer::addSet(int16_t typeIdentifier,
    ...
    PDB_COUT << "TypeID for Set with dbName=" << databaseName << " and setName=" << setName
             << " is " << std::to_string(typeIdentifier) << endl;
    ...
    PDB_COUT << "Time Duration for check registration:\t "
             << std::to_string(
                    std::chrono::duration_cast<std::chrono::duration<float>>(afterChecks - begin)
                        .count())
             << " secs." << endl;
    ...
    PDB_COUT << "Time Duration for broadcastCatalogUpdate DB:\t "
             << std::to_string(std::chrono::duration_cast<std::chrono::duration<float>>(broadCast2 -
                                                                                        broadCast1)
                                   .count())
             << " secs." << endl;
...
bool CatalogServer::deleteDatabase(std::string databaseName, std::string& errMsg) {
    ...
    if (pdbCatalog->getMetadataFromCatalog(
            false, databaseName, vectorResultItems, errMsg, catalogType) == false) {
        errMsg = "Database does not exist.\n";
        return false;
    }
    ...
    if (isMasterCatalogServer) {
        ...
        for (auto& item : updateResults) {
            ...
            PDB_COUT << "DB metadata broadcasted to node IP: " << item.first
                     << ((item.second.first == true) ? " deleted correctly!"
...
CatalogServer::CatalogServer(std::string catalogDirectoryIn,
    ...
    if (pdbCatalog->getMetadataFromCatalog(false,
                                           emptyString,
                                           _udfsValues,
                                           errMsg,
    ...
    if (pdbCatalog->getMetadataFromCatalog(
            false, emptyString, _allDatabases, errMsg, PDBCatalogMsgType::CatalogPDBDatabase) ==
        false)
        PDB_COUT << errMsg << endl;
    ...
    for (int i = 0; i < (*_allDatabases).size(); i++) {
        ...
        for (int j = 0; j < (*(*_allDatabases)[i].getListOfSets()).size(); j++) {
            ...
            PDB_COUT << "Database " << _dbName << " has set " << _setName << " and type "
                     << _typeName << endl;

            // populates information about types and sets for a given database
            PDB_COUT << "Adding type= " << _typeName << " db= " << _dbName << " _set=" << _setName
                     << " typeId= "
                     << string(pdbCatalog->getUserDefinedTypesList()[_typeName].getObjectID())
                     << endl;

            setTypes[make_pair(_dbName, _setName)] = (int16_t)std::atoi(
                pdbCatalog->getUserDefinedTypesList()[_typeName].getObjectID().c_str());
        }
    }

    // retrieves metadata for nodes in the cluster from SQLite storage and loads them into memory
    if (pdbCatalog->getMetadataFromCatalog(
            false, emptyString, _allNodesInCluster, errMsg, PDBCatalogMsgType::CatalogPDBNode) ==
        false)
        PDB_COUT << errMsg << endl;
...
bool CatalogServer::printCatalog(Handle<CatalogPrintMetadata> &metadataToPrint) {
    ...
    if (categoryToPrint.compare("databases")==0)
        pdbCatalog->listRegisteredDatabases(
            resultToPrint,
            errorMsg);

    if (categoryToPrint.compare("sets")==0)
        pdbCatalog->listRegisteredSetsForADatabase(
            resultToPrint,
            itemKey,
            errorMsg);

    if (categoryToPrint.compare("nodes")==0)
        pdbCatalog->listNodesInCluster(
            resultToPrint,
            errorMsg);

    if (categoryToPrint.compare("udts")==0)
        pdbCatalog->listUserDefinedTypes(
            resultToPrint,
            errorMsg);
...
bool CatalogServer::addDatabaseMetadata(Handle<CatalogDatabaseMetadata>& dbMetadata,
                                        std::string& errMsg) {
...
bool CatalogServer::updateDatabaseMetadata(Handle<CatalogDatabaseMetadata>& dbMetadata,
                                           std::string& errMsg) {
...
bool CatalogServer::addSetMetadata(Handle<CatalogSetMetadata>& setMetadata, std::string& errMsg) {
    ...
    *metadataObject = *setMetadata;
    ...
    PDB_COUT << "------>Time Duration to Complete addSetMetadata\t "
             << std::to_string(
                    std::chrono::duration_cast<std::chrono::duration<float>>(beforeReturn - begin)
                        .count())
             << " secs." << endl;
...
bool CatalogServer::addNodeToSet(std::string nodeIP,
                                 std::string databaseName,
                                 std::string setName,
                                 std::string& errMsg) {
    auto begin = std::chrono::high_resolution_clock::now();
    ...
    PDB_COUT << "------>Time Duration to Complete addNodeToSet\t "
             << std::to_string(
                    std::chrono::duration_cast<std::chrono::duration<float>>(addNodeToSet - begin)
                        .count())
             << " secs." << endl;
...
bool CatalogServer::removeNodeFromSet(std::string nodeIP,
    ...
    if (!pdbCatalog->getMetadataFromCatalog(
            false, databaseName, databaseItems, errMsg, catalogType)) {
        PDB_COUT << errMsg << endl;
        return false;
    }
...
bool CatalogServer::broadcastCatalogUpdate(Handle<Type> metadataToSend,
    ...
    for (auto& item : pdbCatalog->getListOfNodesInCluster()) {
        string nodeAddress =
            string(item.second.getNodeIP().c_str()) + ":" + to_string(item.second.getNodePort());
    ...
    PDB_COUT << "------>Time Duration to complete broadcastCatalogUpdate\t"
             << std::to_string(
                    std::chrono::duration_cast<std::chrono::duration<float>>(afterLoop - beforeLoop)
                        .count())
             << " secs." << endl;
...
bool CatalogServer::broadcastCatalogDelete(Handle<Type> metadataToSend,
    ...
    for (auto& item : pdbCatalog->getListOfNodesInCluster()) {
        ...
        if (string(item.second.getNodeType().c_str()).compare("master") != 0) {
            ...
            PDB_COUT << "Don't broadcast to " + nodeAddress + " because it has the master catalog."
                     << endl;
        }
    }
