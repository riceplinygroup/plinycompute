...
int main(int argc, char* argv[]) {
    ...
    */
    ...
    if (whetherToAddData == true) {
        ...
        if (numOfMb > 0) {
            ...
            try {
                ...
                if (!dispatcherClient.sendData<DoubleVector>(
                        std::pair<std::string, std::string>("kmeans_input_set", "kmeans_db"),
                        storeMe,
                        errMsg)) {
                    COUT << "Failed to send data to dispatcher server" << std::endl;
                    return -1;
    ...
    // pdb::Handle<pdb::Vector<DoubleVector>> model = pdb::makeObject<pdb::Vector<DoubleVector>> (k,
    // k);
    ...
    Handle<Computation> myInitialScanSet =
        makeObject<ScanDoubleVectorSet>("kmeans_db", "kmeans_input_set");
    ...
    while (myk < k) {
        ...
        Handle<Computation> myWriteSet =
            makeObject<WriteDoubleVectorSet>("kmeans_db", "kmeans_initial_model_set");
        ...
        SetIterator<DoubleVector> sampleResult =
            myClient.getSetIterator<DoubleVector>("kmeans_db", "kmeans_initial_model_set");
        ...
        if (sampleCount <= k - myk) {  // Assign the sampled values to the model directly
            ...
            COUT << "We got " << sampleCount << " samples. We will randomly select " << (k - myk)
                 << " samples from them." << std::endl;
        ...
        COUT << "After this sampling, we will need another " << (myk >= k ? 0 : k - myk)
             << " samples." << std::endl;
    ...
    for (int n = 0; n < iter; n++) {
        ...
        */
        ...
        // Handle<Computation> myWriteSet = makeObject<WriteKMeansSet>("kmeans_db",
        // "kmeans_output_set");
        ...
        if (ifFirst) {
            for (Handle<KMeansAggregateOutputType> a : result) {
                ...
                COUT << "The cluster count sum I got is " << (*a).getValue().getCount()
                     << std::endl;
    ...
    /*
        COUT << "Initialization Time Duration: " <<
                    std::chrono::duration_cast<std::chrono::duration<float>>(iniEnd-iniBegin).count()
       << " secs." << std::endl;
    ...
    */
    ...
    if (printResult == true) {
        ...
        /*
            SetIterator <DoubleVector> input = myClient.getSetIterator <DoubleVector> ("kmeans_db",
           "kmeans_input_set");
