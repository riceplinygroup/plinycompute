...
int main(int argc, char* argv[]) {
    ...
    if (argc < 8) {
        cout << "bin/CatalogServerTests isMasterCatalog masterIP masterPort localIP localPort "
                "nodeName nodeType"
             << endl;
        cout << "where isMasterCatalog takes a value of 0 to indicate that this is not a node with "
                "the master catalog, 1 otherwise"
             << endl;
        ...
        cout << " example: bin/CatalogServerTests 1 1111.2222.3333.4444 8108 222.222.222.2222 8108 "
                "compute_1 master"
             << endl;
    } else {
    ...
    frontEnd.addFunctionality<pdb::CatalogServer>(
        "CatalogDir", isMasterCatalog, masterIP, masterPort);
