...
int main (int argc, char * argv[]) {
    ...
    if (whetherToAddData == true) {
        ...
        if (!temp.createDatabase ("LA13_db", errMsg)) {
        ...
        if (!temp.createSet<MatrixBlock> ("LA13_db", "LA_input_set", errMsg)) {
        ...
        if (numOfMb > 0) {
            ...
            for (int num = 0; num < numIterations; num++) {
                ...
                try {
                        ...
                        for (int j = 0; j < matrixColNums; j++){
                            ...
                            for(int ii = 0; ii < blockRowNums; ii++){
                                for(int jj=0; jj < blockColNums; jj++){
                                    (*(myData->getRawDataHandle()))[ii*blockColNums+jj] = i+j+ii+jj+0.0;
                                }
                            }
                    ...
                    if (!dispatcherClient.sendData<MatrixBlock>(std::pair<std::string, std::string>("LA_input_set", "LA13_db"), storeMe, errMsg)) {
    ...
    if (!temp.createSet<MatrixBlock> ("LA13_db", "LA_duplicateCol_set", errMsg)) {
        cout << "Not able to create set: " + errMsg;
        exit (-1);
    } else {
        cout << "Created set.\n";
    ...
    LADimension targetDim(5,2,10,4);
    Handle<Computation> myQuery = makeObject<LASillyDuplicateColMultiSelection>(targetDim);
    myQuery->setInput(myScanSet);
    
    Handle<Computation> myWriteSet = makeObject<LAWriteMatrixBlockSet>("LA13_db", "LA_duplicateCol_set");
    ...
    if (printResult == true) {
        ...
        SetIterator <MatrixBlock> input = myClient.getSetIterator <MatrixBlock> ("LA13_db", "LA_input_set");
        ...
        SetIterator <MatrixBlock> result = myClient.getSetIterator <MatrixBlock> ("LA13_db", "LA_duplicateCol_set");
        std :: cout << "ColMax query results: "<< std :: endl;
        int countOut = 0;
        for (auto a : result) {
            countOut ++;
            std :: cout << countOut << ":";
        ...
        std :: cout << "ColMax output count:" << countOut << "\n";
        
    }

    if (clusterMode == false) {
	    // and delete the sets
        myClient.deleteSet ("LA13_db", "LA_duplicateCol_set");
    } else {
        if (!temp.removeSet ("LA13_db", "LA_duplicateCol_set", errMsg)) {
