...
int main (int argc, char * argv[]) {
    ...
    pdb::CatalogClient catalogClient = pdb::CatalogClient(dispatchPort, "localhost", make_shared<pdb::PDBLogger>("clientLog"));
    pdb::theVTable->setCatalogClient(&catalogClient);

    if (argc == 1) {
        std :: cout << "See Test402 usage: ./bin/Test401 dispatcher_port [storage_node_port ... ]" << std::endl;
        return 1;
    }
    ...
    {
        pdb::StorageClient temp(dispatchPort, "localhost", make_shared<pdb::PDBLogger>("clientLog"), usePangaea);
    ...
    for (int i = 0; i < numPorts; i++) {
        int port = ports[i];
        pdb::StorageClient temp2(port, "localhost", make_shared<pdb::PDBLogger>("clientLog"), usePangaea);
        string errMsg;
