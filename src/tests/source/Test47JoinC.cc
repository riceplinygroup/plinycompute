...
public:
	...
	Lambda <bool> getSelection (Handle <int> in1, Handle <StringIntPair> in2, Handle <String> in3, Handle <int> in4) override {
		return (makeLambdaFromSelf (in1) == makeLambdaFromMember (in2, myInt)) && 
			((makeLambdaFromMember (in2, myString) == makeLambdaFromSelf (in3) && makeLambdaFromSelf (in1) == makeLambdaFromSelf (in4)));
	}
	
	Lambda <Handle <String>> getProjection (Handle <int> in1, Handle <StringIntPair> in2, Handle <String> in3, Handle <int> in4) override {
		return makeLambda (in1, in2, in3, in4, [] (Handle <int> &in1, Handle <StringIntPair> &in2, Handle <String> &in3, Handle <int> &in4) {
				std::ostringstream oss; 
				oss << "Got int " << *in1 << " and StringIntPair (" << in2->myInt << ", '" << *(in2->myString) << "') and String '" << *in3 << "' and int " << *in4;
...
int main () {
	...
	// now we create the TCAP string
		...
		AlmostDoneWithEquals (a, c, aAndC, d, bool) <= APPLY (AlmostDoneWithBoth (aExtracted, dExtracted), \n\
			AlmostDoneWithBoth (a, c, aAndC, d), 'JoinComp_3', '==_8') \n\
