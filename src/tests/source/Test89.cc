...
//                            std :: cout << i << ":" << myString << std :: endl;
                       ...
                       //std :: cout << "We comes to " << i << " here" << std :: endl;
                        if (!dispatcherClient.sendData<Supervisor>(std::pair<std::string, std::string>("test89_set", "test89_db"), storeMe, errMsg)) {
        ...
        SetIterator <Supervisor> result = myClient.getSetIterator <Supervisor> ("test89_db", "test89_set");
        int count = 0;
        for (auto a : result)
        {
                     count ++;
	...
	Handle <Computation> myScanSet = makeObject <ScanSupervisorSet> ("test89_db", "test89_set");
        ...
        if (printResult == true) {
            ...
            SetIterator <DepartmentEmployeeAges> result = myClient.getSetIterator <DepartmentEmployeeAges> ("test89_db", "output_set");
