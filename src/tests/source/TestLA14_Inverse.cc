...
int main (int argc, char * argv[]) {
    ...
    if (whetherToAddData == true) {
        ...
        if (!temp.createDatabase ("LA14_db", errMsg)) {
        ...
        if (!temp.createSet<MatrixBlock> ("LA14_db", "LA_input_set", errMsg)) {
                ...
                try {
                    ...
                    for (int i = 0; i < matrixRowNums; i++) {
                        for (int j = 0; j < matrixColNums; j++){
                            pdb :: Handle <MatrixBlock> myData = pdb::makeObject<MatrixBlock>(i,j,blockRowNums,blockColNums,totalRows,totalCols);
                            //Foo initialization
                            for(int ii = 0; ii < blockRowNums; ii++){
                                for(int jj=0; jj < blockColNums; jj++){
                                    (*(myData->getRawDataHandle()))[ii*blockColNums+jj] = (i==j && ii==jj)? i*blockColNums+ii+1:0.0;
                                }
                            }
                    ...
                    if (!dispatcherClient.sendData<MatrixBlock>(std::pair<std::string, std::string>("LA_input_set", "LA14_db"), storeMe, errMsg)) {
    ...
    if (!temp.createSet<MatrixBlock> ("LA14_db", "LA_Inverse_set", errMsg)) {
        cout << "Not able to create set: " + errMsg;
        exit (-1);
    } else {
        cout << "Created set.\n";
    ...
    if (printResult == true) {
        ...
        SetIterator <MatrixBlock> input = myClient.getSetIterator <MatrixBlock> ("LA14_db", "LA_input_set");
        ...
        SetIterator <MatrixBlock> result = myClient.getSetIterator <MatrixBlock> ("LA14_db", "LA_Inverse_set");
    ...
    if (clusterMode == false) {
	    // and delete the sets
        myClient.deleteSet ("LA14_db", "LA_Inverse_set");
    } else {
        if (!temp.removeSet ("LA14_db", "LA_Inverse_set", errMsg)) {
