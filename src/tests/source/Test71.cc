...
int main(int argc, char* argv[]) {
            ...
            for (int num = 0; num < numIterations; num++) {
                ...
                try {
                    for (int i = 0; true; i++) {
                        ...
                        Handle<SimpleEmployee> myData = makeObject<SimpleEmployee>(
                            "Steve Stevens", 20 + (i % 29), i * 34.4, std::string(myString));
                        storeMe->push_back(myData);
                        total++;
                    }

                } catch (pdb::NotEnoughSpace& n) {
                    if (!dispatcherClient.sendData<SimpleEmployee>(
                            std::pair<std::string, std::string>("test71_set", "test71_db"),
                            storeMe,
                            errMsg)) {
    ...
    if (printResult == true) {
        ...
        SetIterator<DepartmentTotal> result =
            myClient.getSetIterator<DepartmentTotal>("test71_db", "output_set1");
