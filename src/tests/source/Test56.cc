...
int main(int argc, char* argv[]) {
    ...
    std::cout << "Usage: #numIterations #printResult[Y/N] #dataSize[MB] #masterIp #addData[Y/N]"
              << std::endl;
    ...
    if (argc > 2) {
        ...
        std::cout << "Will print result. If you don't want to print result, you can add N as the "
                     "second parameter to disable result printing."
                  << std::endl;
    }
        ...
        if (numOfMb > 0) {
            ...
            for (int num = 0; num < numIterations; num++) {
                ...
                try {
                    ...
                    if (!dispatcherClient.sendData<double[NUM_DIMENSIONS]>(
                            std::pair<std::string, std::string>("kmeans_set", "kmeans_db"),
                            storeMe,
                            errMsg)) {
    ...
    Handle<Set<double[NUM_DIMENSIONS]>> myInputSet =
        myClient.getSet<double[NUM_DIMENSIONS]>("kmeans_db", "kmeans_set");
    ...
    Handle<QueryOutput<BuiltinPartialResult>> outputOne =
        makeObject<QueryOutput<BuiltinPartialResult>>("kmeans_db", "output_set1", myQuery);
    ...
    while (numIterated < numIterations) {
        ...
        SetIterator<BuiltinPartialResult> result =
            myClient.getSetIterator<BuiltinPartialResult>("kmeans_db", "output_set1");
        ...
        for (Handle<BuiltinPartialResult> a : result) {
            ...
            for (i = 0; i < NUM_CLUSTERS; i++) {
                newCentroids[i].updateMulti(currentCentroids[i].getSum(),
                                            currentCentroids[i].getCount());
            }
        }
        for (i = 0; i < NUM_CLUSTERS; i++) {
        ...
        std::cout << "Time Duration for the " << numIterated << "-th iteration: "
                  ...
                  << " ns." << std::endl;
        std::cout
            << "Aggregation Duration for the " << numIterated << "-th iteration: "
            << std::chrono::duration_cast<std::chrono::nanoseconds>(aggregationEnd - begin).count()
            << " ns." << std::endl;
        std::cout << "Remove Set Duration for the " << numIterated << "-th iteration: "
                  << std::chrono::duration_cast<std::chrono::nanoseconds>(removeSetEnd -
                                                                          aggregationEnd)
                         .count()
                  << " ns." << std::endl;
        std::cout << "Create Set Duration for the " << numIterated << "-th iteration: "
                  << std::chrono::duration_cast<std::chrono::nanoseconds>(createSetEnd -
                                                                          removeSetEnd)
                         .count()
                  << " ns." << std::endl;
        std::cout << "Create Query Duration for the " << numIterated << "-th iteration: "
                  << std::chrono::duration_cast<std::chrono::nanoseconds>(createQueryEnd -
                                                                          createSetEnd)
                         .count()
                  << " ns." << std::endl;
        std::cout
            << "Execute Query Duration for the " << numIterated << "-th iteration: "
            << std::chrono::duration_cast<std::chrono::nanoseconds>(end - createQueryEnd).count()
            << " ns." << std::endl;
        std::cout << std::endl;
    }
