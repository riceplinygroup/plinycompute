...
if (type == "broadcast") {
    ...
    Handle<Vector<Handle<JoinMap<Object>>>> myMaps = ((Record<Vector<Handle<JoinMap<Object>>>> *)buffer)->getRootObject();
    std :: cout << "number of partitioned maps is " << myMaps->size() << std :: endl;
    for (int i = 0; i < myMaps->size(); i++) {
       std :: cout << "the " << i << "-th partitioned map size is " << (*myMaps)[i]->size() << std :: endl;
    }
}
