...
int main (int argc, char * argv[]) {
    ...
    if (whetherToAddData == true) {
        ...
        if (!temp.createDatabase ("LA11_db", errMsg)) {
        ...
        if (!temp.createSet<MatrixBlock> ("LA11_db", "LA_input_set", errMsg)) {
        ...
        if (numOfMb > 0) {
            ...
            for (int num = 0; num < numIterations; num++) {
                ...
                try {
                    ...
                    if (!dispatcherClient.sendData<MatrixBlock>(std::pair<std::string, std::string>("LA_input_set", "LA11_db"), storeMe, errMsg)) {
    ...
    if (!temp.createSet<MatrixBlock> ("LA11_db", "LA_colMin_set", errMsg)) {
        cout << "Not able to create set: " + errMsg;
        exit (-1);
    } else {
        cout << "Created set.\n";
    ...
    Handle<Computation> myQuery = makeObject<LASillyColMinAggregate>();
    myQuery->setInput(myScanSet);
    //myQuery->setOutput("LA11_db", "LA_colMin_set");
    ...
    if (printResult == true) {
        ...
        SetIterator <MatrixBlock> input = myClient.getSetIterator <MatrixBlock> ("LA11_db", "LA_input_set");
        ...
        SetIterator <MatrixBlock> result = myClient.getSetIterator <MatrixBlock> ("LA11_db", "LA_colMin_set");
        std :: cout << "ColMin query results: "<< std :: endl;
        int countOut = 0;
        for (auto a : result) {
            countOut ++;
            std :: cout << countOut << ":";
        ...
        std :: cout << "ColMin output count:" << countOut << "\n";
        
    }

    if (clusterMode == false) {
	    // and delete the sets
        myClient.deleteSet ("LA11_db", "LA_colMin_set");
    } else {
        if (!temp.removeSet ("LA11_db", "LA_colMin_set", errMsg)) {
