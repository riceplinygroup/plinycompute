...
int main (int argc, char * argv[]) {
        ...
        if (numOfMb > 0) {
            ...
            for (int num = 0; num < numIterations; num++) {
                ...
                try {
                    ...
                    if (!dispatcherClient.sendData<SharedEmployee>(std::pair<std::string, std::string>("input_set", "by8_db"), storeMe, errMsg)) {
    ...
    if (!temp.createSet<SharedEmployee> ("by8_db", "output_set", errMsg)) {
        cout << "Not able to create set: " + errMsg;
        exit (-1);
    } else {
        cout << "Created set.\n";
    ...
    if (printResult == true) {
        ...
        SetIterator <SharedEmployee> result = myClient.getSetIterator <SharedEmployee> ("by8_db", "output_set");
