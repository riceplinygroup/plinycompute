...
int main(int argc, char* argv[]) {
    ...
    if (!temp.createSet<DoubleVector>(
            "gmm_db", "gmm_initial_model_set", errMsg, size_t(16) * size_t(1024) * size_t(1024))) {
    ...
    if (!temp.createSet<GmmAggregateOutputLazy>(
    ...
    for (int currentIter = 0; currentIter < iter; currentIter++) {
        ...
        SetIterator<GmmAggregateOutputLazy> result =
            myClient.getSetIterator<GmmAggregateOutputLazy>("gmm_db", "gmm_output_set");
