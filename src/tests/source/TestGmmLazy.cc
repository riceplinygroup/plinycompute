...
//    srand(time(0));
		...
		if (randomData == true) {
			...
			while (addedData < numData) {
				...
				try {
					...
					//Record<Vector<Handle<Object>>> * myRecord = (Record<Vector<Handle<Object>>> * )getRecord(storeMe);
					//if (!dispatcherClient.sendBytes<DoubleVector>(std::pair<std::string, std::string>("gmm_input_set", "gmm_db"), (char *)myRecord, myRecord->numBytes(), errMsg)) {
	...
	for (int currentIter=0; currentIter < iter; currentIter++){
		...
		iterProcessingOnlyTimes[currentIter] = std::chrono::duration_cast<std::chrono::duration<float>>(end-begin).count();
