...
int main(int argc, char* argv[]) {
    ...
    if (!pdbClient.createSet<DoubleVector>(
            "gmm_db", "gmm_initial_model_set", errMsg)) {
...
makeObject<ScanUserSet<DoubleVector>>("gmm_db", "gmm_input_set");
        ...
        Handle<Computation> myWriteSet =
            makeObject<WriteDoubleVectorSet>("gmm_db", "gmm_initial_model_set");
    ...
    Handle<Computation> myScanSet = makeObject<ScanDoubleVectorSet>("gmm_db", "gmm_input_set");
    Handle<Computation> myDataCount = makeObject<GmmDataCountAggregate>();
    ...
    for (int currentIter = 0; currentIter < iter; currentIter++) {
        ...
        SetIterator<GmmAggregateOutputLazy> result =
                pdbClient.getSetIterator<GmmAggregateOutputLazy>("gmm_db", "gmm_output_set");
        ...
        for (Handle<GmmAggregateOutputLazy> a : result) {
            ...
            std::cout << " previousLogLikelihood: " << previousLogLikelihood
                      << " currentLogLikelihood  " << currentLogLikelihood << std::endl;
        }

        model = currentModel;

        pdbClient.clearSet("gmm_db", "gmm_output_set", "pdb::GmmAggregateOutputLazy", errMsg);
        ...
        COUT << "Server-side Time Duration for Iteration-: " << currentIter << " is:"
             ...
             << " secs." << std::endl;
        COUT
            << "Total Time Duration for Iteration-: " << currentIter << " is:"
    ...
    COUT << "Mean times per iteration ((processing only, no model update, except iter 0):";
