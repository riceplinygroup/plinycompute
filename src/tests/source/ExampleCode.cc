...
#define PAGE_RAW_SIZE (65536 - 512)
    ...
    std::cout << "Are " << getBytesAvailableInCurrentAllocatorBlock()
              << " free bytes in current allocation block.\n";
    std::cout << "Are " << getNumObjectsInCurrentAllocatorBlock()
              << " individual objects in the current block.\n";
    std::cout << "Now I am going to render all of those unreachable via an assign to a nullptr.\n";
    supervisors = nullptr;
    std::cout << "Are " << getNumObjectsInCurrentAllocatorBlock()
              << " individual objects in the current block.\n";
    std::cout << "Are " << getNumObjectsInHomeAllocatorBlock(myEmp)
              << " individual objects in myEmp's block.\n";
