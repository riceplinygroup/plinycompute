	...
	int blockSize = 8;
		...
		for (int docNum = 0; docNum < numDoc; docNum++) {
			...
			for (int i = 0; i <= numWord; i++) {
				...
				try {
					if ((i == numWord && wordsSoFar == 0) || (i != numWord && drand48 () < 0.015)) {
						wordsSoFar++;
					...
					if (!dispatcherClient.sendData<LDADocument>
						(std::pair<std::string, std::string>("LDA_input_set", "LDA_db"), storeMe, errMsg)) {
...
//    auto begin = std :: chrono :: high_resolution_clock :: now();
    for (int n = 0; n < iter; n++) {
		...
		// now we set up the sequence of actions that re-compute the topic probabilities for each doc
		...
		if (!myClient.executeComputations(errMsg, myWriterForTopicsPerWord, myWriterForTopicsPerDoc)) {
			...
			return 1;
                ...
                if (!temp.clearSet ("LDA_db", myReaderForTopicsPerDocSetName, "pdb::IntDoubleVectorPair", errMsg)) {
                        cout << "Not able to create set: " + errMsg;
                        exit (-1);
                }
