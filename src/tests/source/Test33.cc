...
int main (int argc, char * argv[]) {
	...
	std :: cout << "Firstly: Make sure to run bin/test23 or bin/test28 in a different window to provide a catalog/storage server.\n";
        std :: cout << "Secondly: Make sure to run bin/test24 to register the SharedEmployee type once and only once.\n";
	...
	string errMsg;
        ...
        if (!temp.removeSet<SharedEmployee> (databaseName, setName, errMsg)) {
                ...
                std :: cout << "Please change a set name, or remove the pdbRoot AND CatalogDir directories at where you run test23/test28"<< std::endl;
        } else {
