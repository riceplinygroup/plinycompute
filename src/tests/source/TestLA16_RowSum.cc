...
int main (int argc, char * argv[]) {
    ...
    if (whetherToAddData == true) {
        ...
        if (!temp.createDatabase ("LA16_db", errMsg)) {
        ...
        if (!temp.createSet<MatrixBlock> ("LA16_db", "LA_input_set", errMsg)) {
        ...
        if (numOfMb > 0) {
            ...
            for (int num = 0; num < numIterations; num++) {
                ...
                try {
                    ...
                    if (!dispatcherClient.sendData<MatrixBlock>(std::pair<std::string, std::string>("LA_input_set", "LA16_db"), storeMe, errMsg)) {
    ...
    if (!temp.createSet<MatrixBlock> ("LA16_db", "LA_rowSum_set", errMsg)) {
        cout << "Not able to create set: " + errMsg;
        exit (-1);
    } else {
        cout << "Created set.\n";
    ...
    if (printResult == true) {
        ...
        SetIterator <MatrixBlock> input = myClient.getSetIterator <MatrixBlock> ("LA16_db", "LA_input_set");
        ...
        SetIterator <MatrixBlock> result = myClient.getSetIterator <MatrixBlock> ("LA16_db", "LA_rowSum_set");
    ...
    if (clusterMode == false) {
	    // and delete the sets
        myClient.deleteSet ("LA16_db", "LA_rowSum_set");
    } else {
        if (!temp.removeSet ("LA16_db", "LA_rowSum_set", errMsg)) {
