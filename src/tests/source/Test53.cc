...
int main(int argc, char* argv[]) {
    ...
    Handle<KMeansQuery> myFirstSelect = makeObject<KMeansQuery>();
    myFirstSelect->setInput(myInputSet);
    Handle<QueryOutput<PartialResult>> outputOne =
        makeObject<QueryOutput<PartialResult>>("kmeans_db", "output_set1", myFirstSelect);
    ...
    if ((printResult == true) && (clusterMode == false)) {
        SetIterator<PartialResult> result =
            myClient.getSetIterator<PartialResult>("kmeans_db", "output_set1");
