...
int main (int argc, char * argv[]) {
    ...
    if (whetherToAddData == true) {
        ...
        if (!temp.createDatabase ("LA17_db", errMsg)) {
        ...
        if (!temp.createSet<MatrixBlock> ("LA17_db", "LA_input_set", errMsg)) {
        ...
        if (numOfMb > 0) {
            ...
            for (int num = 0; num < numIterations; num++) {
                ...
                try {
                    ...
                    if (!dispatcherClient.sendData<MatrixBlock>(std::pair<std::string, std::string>("LA_input_set", "LA17_db"), storeMe, errMsg)) {
    ...
    if (!temp.createSet<MatrixBlock> ("LA17_db", "LA_colSum_set", errMsg)) {
        cout << "Not able to create set: " + errMsg;
        exit (-1);
    } else {
        cout << "Created set.\n";
    ...
    Handle<Computation> myQuery = makeObject<LASillyColSumAggregate>();
    ...
    if (printResult == true) {
        ...
        SetIterator <MatrixBlock> input = myClient.getSetIterator <MatrixBlock> ("LA17_db", "LA_input_set");
        ...
        SetIterator <MatrixBlock> result = myClient.getSetIterator <MatrixBlock> ("LA17_db", "LA_colSum_set");
    ...
    if (clusterMode == false) {
	    // and delete the sets
        myClient.deleteSet ("LA17_db", "LA_colSum_set");
    } else {
        if (!temp.removeSet ("LA17_db", "LA_colSum_set", errMsg)) {
