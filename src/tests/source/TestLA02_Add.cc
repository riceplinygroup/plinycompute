...
int main (int argc, char * argv[]) {
    ...
    if (whetherToAddData == true) {
        ...
        if (!temp.createDatabase ("LA02_db", errMsg)) {
        ...
        if (!temp.createSet<MatrixBlock> ("LA02_db", "LA_input_set1", errMsg)) {
        ...
        // now, create the first matrix set in that database
        if (!temp.createSet<MatrixBlock> ("LA02_db", "LA_input_set2", errMsg)) {
            cout << "Not able to create set: " + errMsg;
            exit (-1);
        } else {
            cout << "Created set.\n";
        ...
        if (!dispatcherClient.sendData<MatrixBlock>(std::pair<std::string, std::string>("LA_input_set1", "LA02_db"), storeMatrix1, errMsg)) {
        ...
        if (!dispatcherClient.sendData<MatrixBlock>(std::pair<std::string, std::string>("LA_input_set2", "LA02_db"), storeMatrix2, errMsg)) {
            std :: cout << "Failed to send data to dispatcher server" << std :: endl;
            return -1;
        }
        PDB_COUT << total << " MatrixBlock data sent to dispatcher server~~" << std :: endl;
    ...
    if (!temp.createSet<MatrixBlock> ("LA02_db", "LA_sum_set", errMsg)) {
        cout << "Not able to create set: " + errMsg;
        exit (-1);
    } else {
        cout << "Created set.\n";
    ...
    Handle<Computation> myAddJoin = makeObject<LASillyAddJoin>();
    ...
    if (printResult == true) {
        ...
        SetIterator <MatrixBlock> input1 = myClient.getSetIterator <MatrixBlock> ("LA02_db", "LA_input_set1");
        ...
        SetIterator <MatrixBlock> input2 = myClient.getSetIterator <MatrixBlock> ("LA02_db", "LA_input_set2");
        ...
        SetIterator <MatrixBlock> result = myClient.getSetIterator <MatrixBlock> ("LA02_db", "LA_sum_set");
        std :: cout << "Add query results: "<< std :: endl;
        int countOut = 0;
        for (auto a : result) {
            countOut ++;
            std :: cout << countOut << ":";
    ...
    if (clusterMode == false) {
	    // and delete the sets
        myClient.deleteSet ("LA02_db", "LA_sum_set");
    } else {
        if (!temp.removeSet ("LA02_db", "LA_sum_set", errMsg)) {
