...
int main (int argc, char * argv[]) {
       ...
       if (whetherToAddData == true) {
            ...
            if (!temp.createDatabase ("test95_db", errMsg)) {
            ...
            if (!temp.createSet<StringIntPair> ("test95_db", "test95_set1", errMsg)) {
            ...
            if (!temp.createSet<StringIntPair> ("test95_db", "test95_set2", errMsg)) {
                cout << "Not able to create set: " + errMsg;
                exit (-1);
            } else {
            ...
            if (!temp.createSet<StringIntPair> ("test95_db", "test95_set3", errMsg)) {
                cout << "Not able to create set: " + errMsg;
                exit (-1);
            } else {
                ...
                for (int num = 0; num < numIterations; num++) {
                    ...
                    try {
                        for (i=0; true ; i++) {
                            ...
                            oss.str();
                            Handle <StringIntPair> myData = makeObject <StringIntPair> (oss.str(), (i*3));
                        ...
                        if (!dispatcherClient.sendData<StringIntPair>(std::pair<std::string, std::string>("test95_set1", "test95_db"), storeMe, errMsg)) {
                ...
                for (int num = 0; num < numIterations; num++) {
                    ...
                    try {
                        for (i=0; true ; i++) {
                            ...
                            oss.str(); 
                            Handle <StringIntPair> myData = makeObject <StringIntPair> (oss.str(), (i*6));
                            storeMe->push_back(myData);
                            total++;
                        }
                        ...
                        if (!dispatcherClient.sendData<StringIntPair>(std::pair<std::string, std::string>("test95_set2", "test95_db"), storeMe, errMsg)) {
                            std :: cout << "Failed to send data to dispatcher server" << std :: endl;
                            return -1;
                        }
                   }
                ...
                for (int num = 0; num < numIterations; num++) {
                    ...
                    try {
                        for (i=0; true ; i++) {
                            ...
                            oss.str();
                            Handle <StringIntPair> myData = makeObject <StringIntPair> (oss.str(), (i*9));
                            storeMe->push_back(myData);
                            total++;
                        }
                        ...
                        if (!dispatcherClient.sendData<StringIntPair>(std::pair<std::string, std::string>("test95_set3", "test95_db"), storeMe, errMsg)) {
                            std :: cout << "Failed to send data to dispatcher server" << std :: endl;
                            return -1;
                        }
                   }
        ...
        if (!temp.createSet<String> ("test95_db", "output_set1", errMsg)) {
                cout << "Not able to create set: " + errMsg;
                exit (-1);
        } else {
                cout << "Created set.\n";
	...
	// this is the object allocation block where all of this stuff will reside
        ...
        catalogClient.registerType ("libraries/libOptimizedMethodJoin.so", errMsg);
        ...
        catalogClient.registerType ("libraries/libStringIntPairMultiSelection.so", errMsg);
	...
	Handle <Computation> myScanSet1 = makeObject <ScanStringIntPairSet> ("test95_db", "test95_set1");
        ...
        myMultiSelection->setInput(myScanSet1);
	Handle <Computation> myJoin = makeObject <OptimizedMethodJoin> ();
        ...
        if (printResult == true) {
            ...
            SetIterator <String> result = myClient.getSetIterator <String> ("test95_db", "output_set1");
        ...
        if (clusterMode == false) {
            // and delete the sets
            myClient.deleteSet ("test95_db", "output_set1");
        } else {
            if (!temp.removeSet ("test95_db", "output_set1", errMsg)) {
