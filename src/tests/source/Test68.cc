...
/* This test case is an upgrade of Test52, so that the selection ratio can be defined by a parameter */
...
int main (int argc, char * argv[]) {
       ...
       std :: cout << "Usage: #printResult[Y/N] #clusterMode[Y/N] #dataSize[MB] #masterIp #addData[Y/N] #selectionRatio" << std :: endl;        
       if (argc > 1) {
	...
	// register this query class
        catalogClient.registerType ("libraries/libChrisSelection.so", errMsg);
	...
	Handle <ChrisSelection> myQuery = makeObject <ChrisSelection> ();
	myQuery->setInput (myInputSet);
	Handle <QueryOutput <String>> outputOne = makeObject <QueryOutput <String>> ("chris_db", "output_set1", myQuery);
