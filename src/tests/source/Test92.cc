...
int main (int argc, char * argv[]) {
       ...
       if (whetherToAddData == true) {
            ...
            if (!temp.createDatabase ("test92_db", errMsg)) {
            ...
            if (!temp.createSet<StringIntPair> ("test92_db", "test92_set1", errMsg)) {
            ...
            if (!temp.createSet<StringIntPair> ("test92_db", "test92_set2", errMsg)) {
                cout << "Not able to create set: " + errMsg;
                exit (-1);
            } else {
            ...
            if (!temp.createSet<String> ("test92_db", "test92_set3", errMsg)) {
                cout << "Not able to create set: " + errMsg;
                exit (-1);
            } else {
                ...
                for (int num = 0; num < numIterations; num++) {
                    ...
                    try {
                        for (i=0; true ; i++) {
                            ...
                            oss << "My string is " << (i*3)%10000;
                            oss.str();
                            Handle <StringIntPair> myData = makeObject <StringIntPair> (oss.str(), (i*3)%10000);
                        ...
                        if (!dispatcherClient.sendData<StringIntPair>(std::pair<std::string, std::string>("test92_set1", "test92_db"), storeMe, errMsg)) {
            ...
            if (numOfMb > 0) {
                ...
                for (int num = 0; num < numIterations; num++) {
                    ...
                    try {
                        for (i=0; true ; i++) {
                            std :: ostringstream oss;
                            oss << "My string is " << (i*6)%10000;
                            oss.str(); 
                            Handle <StringIntPair> myData = makeObject <StringIntPair> (oss.str(), (i*6)%10000);
                            storeMe->push_back(myData);
                            total++;
                        }
                        ...
                        if (!dispatcherClient.sendData<StringIntPair>(std::pair<std::string, std::string>("test92_set2", "test92_db"), storeMe, errMsg)) {
                            std :: cout << "Failed to send data to dispatcher server" << std :: endl;
                            return -1;
                        }
                   }
            ...
            if (numOfMb > 0) {
                ...
                for (int num = 0; num < numIterations; num++) {
                    ...
                    try {
                        for (i=0; true ; i++) {
                            std :: ostringstream oss;
                            oss << "My string is " << (i*9)%10000;
                            oss.str();
                            Handle <StringIntPair> myData = makeObject <StringIntPair> (oss.str(), (i*9)%10000);
                            storeMe->push_back(myData);
                            total++;
                        }
                        ...
                        if (!dispatcherClient.sendData<StringIntPair>(std::pair<std::string, std::string>("test92_set3", "test92_db"), storeMe, errMsg)) {
                            std :: cout << "Failed to send data to dispatcher server" << std :: endl;
                            return -1;
                        }
                   }
        ...
        if (!temp.createSet<String> ("test92_db", "output_set1", errMsg)) {
                cout << "Not able to create set: " + errMsg;
                exit (-1);
        } else {
                cout << "Created set.\n";
	...
	// this is the object allocation block where all of this stuff will reside
        ...
        catalogClient.registerType ("libraries/libMethodJoin.so", errMsg);
	...
	Handle <Computation> myScanSet1 = makeObject <ScanStringIntPairSet> ("test92_db", "test92_set1");
        Handle <Computation> myScanSet2 = makeObject <ScanStringIntPairSet> ("test92_db", "test92_set2");
        Handle <Computation> myScanSet3 = makeObject <ScanStringIntPairSet> ("test92_db", "test92_set3");
	Handle <Computation> myJoin = makeObject <MethodJoin> ();
        myJoin->setInput(0, myScanSet1);
        myJoin->setInput(1, myScanSet2);
        Handle <Computation> myOtherJoin = makeObject <MethodJoin> ();
        ...
        Handle <Computation> myWriter = makeObject<WriteStringSet>("test92_db", "output_set1");
        ...
        if (printResult == true) {
            ...
            SetIterator <String> result = myClient.getSetIterator <String> ("test92_db", "output_set1");
        ...
        if (clusterMode == false) {
            // and delete the sets
            myClient.deleteSet ("test92_db", "output_set1");
        } else {
            if (!temp.removeSet ("test92_db", "output_set1", errMsg)) {
