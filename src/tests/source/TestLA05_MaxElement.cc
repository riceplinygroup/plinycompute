...
int main (int argc, char * argv[]) {
    ...
    if (whetherToAddData == true) {
        ...
        if (!temp.createDatabase ("LA05_db", errMsg)) {
        ...
        if (!temp.createSet<MatrixBlock> ("LA05_db", "LA_input_set", errMsg)) {
        ...
        if (numOfMb > 0) {
            ...
            for (int num = 0; num < numIterations; num++) {
                ...
                try {
                    ...
                    if (!dispatcherClient.sendData<MatrixBlock>(std::pair<std::string, std::string>("LA_input_set", "LA05_db"), storeMe, errMsg)) {
    ...
    if (!temp.createSet<MatrixBlock> ("LA05_db", "LA_max_set", errMsg)) {
        cout << "Not able to create set: " + errMsg;
        exit (-1);
    } else {
        cout << "Created set.\n";
    ...
    Handle<Computation> myQuery = makeObject<LASillyMaxElementAggregate>();
    ...
    if (printResult == true) {
        ...
        SetIterator <MatrixBlock> input = myClient.getSetIterator <MatrixBlock> ("LA05_db", "LA_input_set");
        ...
        SetIterator <LAMaxElementOutputType> result = myClient.getSetIterator <LAMaxElementOutputType> ("LA05_db", "LA_max_set");
    ...
    if (clusterMode == false) {
	    // and delete the sets
        myClient.deleteSet ("LA05_db", "LA_max_set");
    } else {
        if (!temp.removeSet ("LA05_db", "LA_max_set", errMsg)) {
