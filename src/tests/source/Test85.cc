...
int main (int argc, char * argv[]) {
       ...
       if (whetherToAddData == true) {
            ...
            if (!temp.createDatabase ("test85_db", errMsg)) {
            ...
            if (!temp.createSet<Supervisor> ("test85_db", "test85_set", errMsg)) {
            ...
            if (numOfMb > 0) {
                ...
                for (int num = 0; num < numIterations; num++) {
                    ...
                    try {
                        ...
                        if (!dispatcherClient.sendData<Supervisor>(std::pair<std::string, std::string>("test85_set", "test85_db"), storeMe, errMsg)) {
        ...
        if (!temp.createSet<Employee> ("test85_db", "output_set1", errMsg)) {
                cout << "Not able to create set: " + errMsg;
                exit (-1);
        } else {
                cout << "Created set.\n";
        ...
        PDB_COUT << "to create a new set for storing output data" << std :: endl;
        if (!temp.createSet<DepartmentTotal> ("test85_db", "output_set2", errMsg)) {
                cout << "Not able to create set: " + errMsg;
                exit (-1);
        } else {
                cout << "Created set.\n";
	...
	Handle <Computation> myWriteSet = makeObject <WriteBuiltinEmployeeSet> ("test85_db", "output_set1");
	myWriteSet->setInput(myFilter);

        auto begin = std :: chrono :: high_resolution_clock :: now();

        if (!myClient.executeComputations(errMsg, myWriteSet, myAgg)) {
        ...
        if (printResult == true) {
            ...
            SetIterator <Employee> result = myClient.getSetIterator <Employee> ("test85_db", "output_set1");
        ...
        if (printResult == true) {
            std :: cout << "to print result..." << std :: endl;
            SetIterator <DepartmentTotal> result = myClient.getSetIterator <DepartmentTotal> ("test85_db", "output_set2");
        ...
        if (clusterMode == false) {
            // and delete the sets
            myClient.deleteSet ("test85_db", "output_set1");
            myClient.deleteSet ("test85_db", "output_set2");
        } else {
            if (!temp.removeSet ("test85_db", "output_set1", errMsg)) {
                cout << "Not able to remove set: " + errMsg;
                exit (-1);
            } else {
                cout << "Removed set.\n";
            }
            if (!temp.removeSet ("test85_db", "output_set2", errMsg)) {
                cout << "Not able to remove set: " + errMsg;
                exit (-1);
            } else {
                cout << "Removed set.\n";
