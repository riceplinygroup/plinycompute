...
namespace pdb_detail
{
    ...
    /**
     * An instruction that describes how to create a new table from an existing table by extracting a field value
     ...
     * Result using comp.
     *
     * In this example the Hoist instruction that described creation of the Result table above would have the
     * following form:
     *
     ...
     * Result should contain not just the result column from comp, but also a copy of the Person column from the
     * Persons table.
     *
    ...
    private:
         ...
         *
         * @param fieldId  A metadata field to describe the field being hoisted. Does not influence the Hoist operation.
         ...
         */
        Hoist(const string &fieldId, TableColumn inputColumn, TableColumn outputColumn,
              shared_ptr<vector<TableColumn>> columnsToCopyToOutputTable, const string &executorId);

        // for constructor
        friend shared_ptr<Hoist> makeHoist(const string &fieldId, TableColumn inputColumn, TableColumn outputColumn,
