...
/**
 * An instruction that describes how to create a new table from an existing table by filtering the
 * rows of the
 ...
 * ----------|----------
 *
 * a filter operation could consider the column isOdd to produce an output table "Output" of the
 * form:
 *
 *
 * ----------|----------
 ...
 *
 *
 * In this example the filter instruction that described creation of the output table above would
 * have the
 * following form:
 *
 * inputTableId would be "Input" to encode that the input table was the source of consideration.
 *
 * filterColumnId would be "isOdd" to encode that row should be retained or disguarded based on
 * their value
 * in the isOdd column.
 *
 * columnsToCopyToOutputTable would have the value [("Input", "Num"), ("Input", "isOdd")] to encode
 * that
...
private:
     ...
     * @param columnsToCopyToOutputTable any columns to copy from the output table to the input
     * table
     * @return a new filter instruction
     */
    // private because throws exception and PDB style guide disallows exceptions from crossing API
    // boundaries
    Filter(const string& inputTableId,
           const string& filterColumnId,
           const string& outputTableId,
           shared_ptr<vector<TableColumn>> columnsToCopyToOutputTable);

    // for constructor
    friend shared_ptr<Filter> makeFilter(
        const string& inputTableId,
        const string& filterColumnId,
        const string& outputTableId,
        shared_ptr<vector<TableColumn>> columnsToCopyToOutputTable);
