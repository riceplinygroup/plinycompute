...
{
    /**
     * An instruction that describes how to create a new table from existing table(s) by application of an
     * externally defined executor that can compare values using a "greater than" operation.
     *
     * For example, consider an executor "comp" that consumes two integer columns from two tables to produce a
     * new result column.
     *
     ...
     *
     *
     * An GreaterThan instruction could the be created to decribe how to take the input tables and produce
     * Result.
     *
     * In this example the GreaterThan instruction that described creation of the Result table above would have the
     * following form:
     *
     ...
     *
     * outputColumn would have the value ("Result", "hasLittleSister") to denote the name of the table to be created
     ...
     * Result should contain not just the result column from sum, but also a copy of the Name column from the Brother
     * table.
     *
     */
    ...
    private:
         ...
         * @param columnsToCopyToOutputTable Any optional columns to copy into the output table during its contruction.
         ...
         */
        GreaterThan(TableColumn leftHandSide, TableColumn rightHandSide, TableColumn outputColumn,
                    shared_ptr<vector<TableColumn>> columnsToCopyToOutputTable, const string &executorId);

        // for constructor
        friend  shared_ptr<GreaterThan> makeGreaterThan(TableColumn leftHandSide, TableColumn rightHandSide,
