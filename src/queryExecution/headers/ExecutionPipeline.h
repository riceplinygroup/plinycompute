...
public:
	...
	~ExecutionPipeline () {
		...
		// write back all of the pages
		cleanPages (999999999, finalOutputStage);
		storage->stopFlushConsumerThreads();
		if (unwrittenPages.size () != 0)
			std :: cout << "This is bad: in destructor for pipeline, still some pages with objects!!\n";
	}
	...
	// writes back any unwritten pages
	void cleanPages (int iteration, int sinkStage) {
							...
							for (int stage = inputNodeId + 1; stage < nextInputNodeId; ++stage) {
								...
								if (nodeid2Outputs.count(stage) != 0) {
									...
									//try {
										...
										auto outputs = nodeid2Outputs[stage];
										for (auto oneOutput : outputs) {
											SetPtr outputSet = storage->getSet(std :: pair <std :: string, std :: string> (oneOutput.getdbName(), oneOutput.getSetName()));
									        storage->getCache()->pin(outputSet, MRU, Write);
						        			PDBPagePtr outputPage = storage->getNewPage(std :: pair <std :: string, std :: string>(oneOutput.getdbName(), oneOutput.getSetName()));
						        			
											myRAM = std :: make_shared <My_MemoryHolder> (std :: make_pair(outputPage->getBytes(), outputPage->getSize()));
											outputVector = myRAM->getOutputVector (curChunk, whichColToOutput);
											curChunk->writeOutColumn (whichColToOutput, outputVector, false);
...
/
									// lastly, write back all of the output pages
									finalOutputStage = stage;
									cleanPages (iteration, stage);
