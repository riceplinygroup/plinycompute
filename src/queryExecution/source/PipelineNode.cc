...
PipelineNode::PipelineNode(NodeID nodeId,
...
bool PipelineNode::unbundle(PipelineContextPtr context,
    ...
    while (unbundler->fillNextOutputVector()) {
        ...
        if (outputVecSize > 0) {
            ...
            // unpin output page
            context->getProxy()->unpinUserPage(nodeId,
            ...
            context->getProxy()->addUserPage(context->getOutputSet()->getDatabaseId(),
                                             context->getOutputSet()->getTypeId(),
                                             context->getOutputSet()->getSetId(),
                                             output);
            PDB_COUT << "pinned page in output set with id=" << output->getPageID() << std::endl;
            logger->debug(std::string("pinned page in output set with id=") +
...
bool PipelineNode::run(PipelineContextPtr context,
                       Handle<GenericBlock> inputBatch,
                       size_t batchSize,
                       PDBLoggerPtr logger) {
#ifdef DEBUG_PIPELINE
    ...
    try {
        ...
        PDB_COUT << "PipelineNode::run()--loadOutputBlock before loop: current block is full, copy "
                    "to output page!"
                 << std::endl;
        logger->debug(
            std::string("PipelineNode::run()--loadOutputBlock before loop: current block is full, "
                        "copy to output page!"));
        size_t outputVecSize = context->getOutputVec()->size();
        if (outputVecSize > 0) {
            // copy data to output page
            ...
            PDB_COUT << "to add user page for output" << std::endl;
            logger->debug(std::string("PipelineNode: to add user page for output"));
            proxy->addUserPage(context->getOutputSet()->getDatabaseId(),
                               context->getOutputSet()->getTypeId(),
                               context->getOutputSet()->getSetId(),
                               output);
            PDB_COUT << "pinned page in output set with id=" << output->getPageID() << std::endl;
            logger->debug(std::string("PipelineNode: pinned page in output set with id=") +
                          std::to_string(output->getPageID()));
            context->setPageToUnpin(output);
        }
        std::string out = getAllocator().printInactiveBlocks();
    ...
    while (processor->fillNextOutputBlock()) {
        ...
        if (context->isOutputFull()) {
            PDB_COUT << "PipelineNode::run()--fillNextOutputBlock(): current block is full, copy "
                        "to output page!"
                     << std::endl;
            logger->debug(
                std::string("PipelineNode::run()--fillNextOutputBlock(): current block is full, "
                            "copy to output page!"));
            size_t outputVecSize = context->getOutputVec()->size();
            if (outputVecSize > 0) {
                // copy data to output page
                ...
                proxy->addUserPage(context->getOutputSet()->getDatabaseId(),
                                   ...
                                   context->getOutputSet()->getSetId(),
                                   output);
                PDB_COUT << "pinned page in output set with id=" << output->getPageID()
                         << std::endl;
                logger->debug(std::string("PipelineNode: pinned page in output set with id=") +
                              std::to_string(output->getPageID()));
                context->setPageToUnpin(output);
            ...
            try {
                ...
                PDB_COUT
                    << "PipelineNode: deep copy unfinished object meet problem when deep copy again"
                    << std::endl;
        ...
        for (int i = 0; i < this->children->size(); i++) {
            ...
            logger->debug(std::to_string(id) + std::string(":run ") + std::to_string(i) +
                          std::string("-th child"));
#endif
            children->at(i)->run(context, outputBlock, batchSize, logger);
#ifdef DEBUG_PIPELINE
            PDB_COUT << id << ": done" << i << "-th child" << std::endl;
            logger->debug(std::to_string(id) + std::string(":done ") + std::to_string(i) +
                          std::string("-th child"));
#endif
        }
...
// I am a sink node, run unbundling
        ...
        try {
            ...
            PDB_COUT << "PipelineNode::run()--loadOutputBlock() in loop: current block is full, "
                        "copy to output page!"
                     << std::endl;
            logger->debug(
                std::string("PipelineNode::run()--loadOutputBlock() in loop: current block is "
                            "full, copy to output page!"));
            size_t outputVecSize = context->getOutputVec()->size();
            if (outputVecSize > 0) {
                // copy data to output page
                Record<Vector<Handle<Object>>>* myBytes = getRecord(context->getOutputVec());
    ...
    logger->debug(std::string("PipelineNode:") + std::to_string(id) +
                  std::string(": we processed the input block"));
#endif
