...
int main(int argc, char* argv[]) {
    ...
    if (whetherToAddData == true) {
        ...
        if (!pdbClient.createDatabase("LA17_db");

        // now, create a new set in that database
        if (!pdbClient.createSet<MatrixBlock>("LA17_db", "LA_input_set");
    ...
    if (!pdbClient.createSet<MatrixBlock>("LA17_db", "LA_colSum_set");
    ...
    if (clusterMode == false) {
        ...
        if (!pdbClient.removeSet("LA17_db", "LA_colSum_set");
    }
