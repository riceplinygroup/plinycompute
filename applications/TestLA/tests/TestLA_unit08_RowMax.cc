...
int main(int argc, char* argv[]) {
    ...
    if (whetherToAddData == true) {
        ...
        if (!pdbClient.createDatabase("LA08_db");

        // now, create a new set in that database
        if (!pdbClient.createSet<MatrixBlock>("LA08_db", "LA_input_set");
    ...
    if (!pdbClient.createSet<MatrixBlock>("LA08_db", "LA_rowMax_set");
    ...
    pdbClient.registerType("libraries/libLARowMaxAggregate.so", errMsg);
    ...
    if (clusterMode == false) {
        ...
        if (!pdbClient.removeSet("LA08_db", "LA_rowMax_set");
    }
