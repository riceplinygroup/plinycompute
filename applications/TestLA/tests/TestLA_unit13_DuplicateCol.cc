...
int main(int argc, char* argv[]) {
    ...
    if (whetherToAddData == true) {
        ...
        if (!pdbClient.createDatabase("LA13_db");

        // now, create a new set in that database
        if (!pdbClient.createSet<MatrixBlock>("LA13_db", "LA_input_set");
    ...
    if (!pdbClient.createSet<MatrixBlock>("LA13_db", "LA_duplicateCol_set");
    ...
    if (clusterMode == false) {
        ...
        if (!pdbClient.removeSet("LA13_db", "LA_duplicateCol_set");
    }
