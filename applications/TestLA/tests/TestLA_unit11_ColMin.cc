...
int main(int argc, char* argv[]) {
    ...
    if (whetherToAddData == true) {
        ...
        if (!pdbClient.createDatabase("LA11_db");

        // now, create a new set in that database
        if (!pdbClient.createSet<MatrixBlock>("LA11_db", "LA_input_set");
    ...
    if (!pdbClient.createSet<MatrixBlock>("LA11_db", "LA_colMin_set");
    ...
    if (clusterMode == false) {
        ...
        if (!pdbClient.removeSet("LA11_db", "LA_colMin_set");
    }
