...
int main(int argc, char* argv[]) {
    ...
    if (whetherToAddData == true) {
        ...
        if (!pdbClient.createDatabase("LA16_db");

        // now, create a new set in that database
        if (!pdbClient.createSet<MatrixBlock>("LA16_db", "LA_input_set");
    ...
    if (!pdbClient.createSet<MatrixBlock>("LA16_db", "LA_rowSum_set");
    ...
    if (clusterMode == false) {
        ...
        if (!pdbClient.removeSet("LA16_db", "LA_rowSum_set");
    }
