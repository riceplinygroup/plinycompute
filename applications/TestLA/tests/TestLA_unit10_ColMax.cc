...
int main(int argc, char* argv[]) {
    ...
    if (whetherToAddData == true) {
        ...
        if (!pdbClient.createDatabase("LA10_db");

        // now, create a new set in that database
        if (!pdbClient.createSet<MatrixBlock>("LA10_db", "LA_input_set");
    ...
    if (!pdbClient.createSet<MatrixBlock>("LA10_db", "LA_colMax_set");
    ...
    if (clusterMode == false) {
        ...
        if (!pdbClient.removeSet("LA10_db", "LA_colMax_set");
    }
