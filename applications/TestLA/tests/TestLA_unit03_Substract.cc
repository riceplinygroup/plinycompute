...
int main(int argc, char* argv[]) {
    ...
    if (whetherToSubstractData == true) {
        ...
        if (!pdbClient.createDatabase("LA03_db");

        // now, create the first matrix set in that database
        if (!pdbClient.createSet<MatrixBlock>("LA03_db", "LA_input_set1");

        // now, create the first matrix set in that database
        if (!pdbClient.createSet<MatrixBlock>("LA03_db", "LA_input_set2");
    ...
    if (!pdbClient.createSet<MatrixBlock>("LA03_db", "LA_diff_set");
    ...
    if (clusterMode == false) {
        ...
        if (!pdbClient.removeSet("LA03_db", "LA_diff_set");
    }
