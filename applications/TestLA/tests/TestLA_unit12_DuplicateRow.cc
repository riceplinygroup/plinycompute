...
int main(int argc, char* argv[]) {
    ...
    if (whetherToAddData == true) {
        ...
        if (!pdbClient.createDatabase("LA12_db");

        // now, create a new set in that database
        if (!pdbClient.createSet<MatrixBlock>("LA12_db", "LA_input_set");
    ...
    if (!pdbClient.createSet<MatrixBlock>("LA12_db", "LA_duplicateRow_set");
    ...
    if (clusterMode == false) {
        ...
        if (!pdbClient.removeSet("LA12_db", "LA_duplicateRow_set");
    }
