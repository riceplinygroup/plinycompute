...
int main(int argc, char* argv[]) {
    ...
    if (whetherToAddData == true) {
        ...
        if (!pdbClient.createDatabase("test93_db", errMsg)) {
        ...
        if (!pdbClient.createSet<StringIntPair>("test93_db", "test93_set1", errMsg)) {
        ...
        if (!pdbClient.createSet<StringIntPair>("test93_db", "test93_set2", errMsg)) {
            cout << "Not able to create set: " + errMsg;
            exit(-1);
        } else {
        ...
        if (!pdbClient.createSet<StringIntPair>("test93_db", "test93_set3", errMsg)) {
            cout << "Not able to create set: " + errMsg;
            exit(-1);
        } else {
    ...
    if (!pdbClient.createSet<String>("test93_db", "output_set1", errMsg)) {
        cout << "Not able to create set: " + errMsg;
        exit(-1);
    } else {
        cout << "Created set.\n";
    ...
    if (printResult == true) {
        ...
        SetIterator<String> result = pdbClient.getSetIterator<String>("test93_db", "output_set1");
    ...
    if (clusterMode == false) {
        // and delete the sets
        pdbClient.deleteSet("test93_db", "output_set1");
    } else {
        if (!pdbClient.removeSet("test93_db", "output_set1", errMsg)) {
